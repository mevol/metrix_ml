...
def make_output_folder(outdir):
    ...
    Args:
        ...
        output_dir (str): the newly created output directory named
                         "linear_svm_randomsearch"
    Yields:
        directory
    '''
    output_dir = os.path.join(outdir, 'linear_svm_randomsearch')
    os.makedirs(output_dir, exist_ok=True)
    return output_dir

class LinearSvmRandSearch():
    ''' A class to conduct a randomised search and training for best parameters for a
        SVM classifier with linear kernel; the following steps are executed:
        ...
        * get 95% confidence interval for calibrated classifier
        ...
        logfile: "linear_svm_randomsearch.log"
        plots: "bootstrap_hist_uncalibrated_<date>.png"
               "feature_importances_coefficient_<date>.png"
    ...
    def __init__(self, data, directory, numf, numc, cv, bootiter):
        ...
        logging.basicConfig(level=logging.INFO, filename=os.path.join(self.directory,
                    'linear_svm_randomsearch.log'), filemode='w')
...
def run(input_csv, output_dir, features, cycles, boot_iter, cv):
    ...
    LinearSvmRandSearch(input_csv, output_dir, features, cycles, boot_iter, cv)



def main():
    '''defining the command line input to make it runable'''
    parser = argparse.ArgumentParser(description='SVM linear kernel randomized search')
