...
class DecisionTreeRandomSearchDatabase(object):
...
#                      'RmergeI', 'RmeasI', 'RmeasdiffI', 'RpimdiffI', 'RpimI', 'diffF']
    ...
    attr_database_normal = ['anomalousmulti', 'anomalousslope', 'lowreslimit',
                      'totalunique', 'RpimdiffI']

    attr_database_stratified = ['IoverSigma', 'wilsonbfactor', 'totalunique', 'RpimdiffI', 'diffF']


    metrix_database = self.metrix[attr_database_stratified]
    
    with open(os.path.join(self.database_features, 'decisiontree_randomsearch.txt'), 'a') as text_file:
...
#stratified split of samples
  ...
  def tree_best_params(self):
    ...
    plot_features(self.tree_clf_rand_new_database, self.X_database_train.columns, self.database_features)
    ...
    write_pickle(self.tree_clf_rand_new_database, self.database_features, 'database')
    ...
    visualise_tree(self.tree_clf_rand_new_database, self.database_features, self.X_database_train.columns, 'database')
    ...
    basic_stats(self.tree_clf_rand_new_database, self.X_database_train, self.database_features)
  ...
  def analysis(self):
    ...
    prediction_probas(self.tree_clf_rand_new_database, self.X_database_train, self.y_train, self.X_database_test, self.y_test, self.y_pred_proba_database, self.y_train_CV_pred_proba_database, self.database_features, 'database')    
...
def run():
  ...
  decision_tree_random_search_database = DecisionTreeRandomSearchDatabase(metrix, database_features, bbbb)
