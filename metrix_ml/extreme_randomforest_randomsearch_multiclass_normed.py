...
def make_output_folder(outdir):
    ...
    Args:
        ...
        output_dir (str): the newly created output directory named
                         "extreme_randomforest_randomsearch_normed"
    Yields:
        directory
    '''
    output_dir = os.path.join(outdir, 'extreme_randomforest_randomsearch_normed')
    os.makedirs(output_dir, exist_ok=True)
    return output_dir

class ExtremeRandomForestRandSearch():
    ''' A class to conduct a randomised search and training for best parameters for a
        Extreme Random Forest for multiple classes; the following steps are executed:
