...
#found for the base_estimator which is a DecisionTree
  ...
  def tree_best_params(self):
    ...
    self.tree_clf_grid_bag_new_database = BaggingClassifier(
                                        DecisionTreeClassifier(**self.best_params_database, random_state=42),
                                        n_jobs=-1, bootstrap=True, random_state=42, n_estimators=100)
    self.tree_clf_grid_bag_new_database.fit(self.X_database_train, self.y_train)
    
    self.tree_clf_grid_bag_new_man_add = BaggingClassifier(
                                        DecisionTreeClassifier(**self.best_params_man_add, random_state=42),
                                        n_jobs=-1, bootstrap=True, random_state=42, n_estimators=100)
    self.tree_clf_grid_bag_new_man_add.fit(self.X_man_add_train, self.y_train)
    
    self.tree_clf_grid_bag_new_transform = BaggingClassifier(
                                        DecisionTreeClassifier(**self.best_params_transform, random_state=42),
                                        n_jobs=-1, bootstrap=True, random_state=42, n_estimators=100)
    self.tree_clf_grid_bag_new_transform.fit(self.X_transform_train, self.y_train)
    ...
    feature_importances(self.tree_clf_grid_bag_new_man_add, self.X_man_add_train, self.man_add)
    feature_importances(self.tree_clf_grid_bag_new_transform, self.X_transform_train, self.transform)
    feature_importances(self.tree_clf_grid_bag_new_prot_screen_trans, self.X_prot_screen_trans_train, self.prot_screen_trans)
    ...
    write_pickle(self.tree_clf_grid_bag_new_database, self.database, 'database')
    write_pickle(self.tree_clf_grid_bag_new_man_add, self.man_add, 'man_add')
    write_pickle(self.tree_clf_grid_bag_new_transform, self.transform, 'transform')
    write_pickle(self.tree_clf_grid_bag_new_prot_screen_trans, self.prot_screen_trans, 'prot_screen_trans')
    ...
    visualise_tree(self.tree_clf_grid_bag_new_database, self.database, self.X_database_train.columns, 'database')
    visualise_tree(self.tree_clf_grid_bag_new_man_add, self.man_add, self.X_man_add_train.columns, 'man_add')
    visualise_tree(self.tree_clf_grid_bag_new_transform, self.transform, self.X_transform_train.columns, 'transform')
    visualise_tree(self.tree_clf_grid_bag_new_prot_screen_trans, self.prot_screen_trans, self.X_prot_screen_trans_train.columns, 'prot_screen_trans')
    ...
    basic_stats(self.tree_clf_grid_bag_new_database, self.X_database_train, self.database)
    basic_stats(self.tree_clf_grid_bag_new_man_add, self.X_man_add_train, self.man_add)
    basic_stats(self.tree_clf_grid_bag_new_transform, self.X_transform_train, self.transform)
    basic_stats(self.tree_clf_grid_bag_new_prot_screen_trans, self.X_prot_screen_trans_train, self.prot_screen_trans)
  ...
  def predict(self):
    ...
    #self.y_pred_database = self.tree_clf_grid_bag_new_database.predict(self.X_database_test)
    self.y_pred_database = self.tree_clf_grid_bag_new_database.predict(self.X_database_test)
    self.y_pred_proba_database = self.tree_clf_grid_bag_new_database.predict_proba(self.X_database_test)
    ...
    #self.y_pred_man_add = self.tree_clf_grid_bag_new_man_add.predict(self.X_man_add_test)
    self.y_pred_man_add = self.tree_clf_grid_bag_new_man_add.predict(self.X_man_add_test)
    self.y_pred_proba_man_add = self.tree_clf_grid_bag_new_man_add.predict_proba(self.X_man_add_test)
    ...
    #self.y_pred_transform = self.tree_clf_grid_bag_new_transform.predict(self.X_transform_test)
    self.y_pred_transform = self.tree_clf_grid_bag_new_transform.predict(self.X_transform_test)
    self.y_pred_proba_transform = self.tree_clf_grid_bag_new_transform.predict_proba(self.X_transform_test)
    ...
    self.y_train_CV_pred_database = cross_val_predict(self.tree_clf_grid_bag_new_database, self.X_database_train, self.y_train, cv=10)
    self.y_train_CV_pred_proba_database = cross_val_predict(self.tree_clf_grid_bag_new_database, self.X_database_train, self.y_train, cv=10, method='predict_proba')
    with open(os.path.join(self.database, 'decisiontree_bag_randomsearch.txt'), 'a') as text_file:
      text_file.write('Saving predictions and probabilities for X_database_train with 10-fold CV in y_train_pred_database \n')
    self.y_train_CV_pred_man_add = cross_val_predict(self.tree_clf_grid_bag_new_man_add, self.X_man_add_train, self.y_train, cv=10)
    self.y_train_CV_pred_proba_man_add = cross_val_predict(self.tree_clf_grid_bag_new_man_add, self.X_man_add_train, self.y_train, cv=10, method='predict_proba')
    with open(os.path.join(self.man_add, 'decisiontree_bag_randomsearch.txt'), 'a') as text_file:
      text_file.write('Saving predictions and probabilities for X_man_add_train with 10-fold CV in y_train_pred_man_add \n')
    self.y_train_CV_pred_transform = cross_val_predict(self.tree_clf_grid_bag_new_transform, self.X_transform_train, self.y_train, cv=10)
    self.y_train_CV_pred_proba_transform = cross_val_predict(self.tree_clf_grid_bag_new_transform, self.X_transform_train, self.y_train, cv=10, method='predict_proba')
    with open(os.path.join(self.transform, 'decisiontree_bag_randomsearch.txt'), 'a') as text_file:
...
#        plt.savefig(os.path.join(directory, 'Precision_Recall_curve_skplt_tree_grid_'+name+datestring+'.png'))
#        plt.close()
#
...
#      precision_recall(self.y_train, y_train_CV_pred_proba, 'train_CV_', directory)
    ...
    prediction_probas(self.tree_clf_grid_bag_new_database, self.X_database_train, self.y_train, self.X_database_test, self.y_test, self.y_pred_proba_database, self.y_train_CV_pred_proba_database, self.database, 'database')    
    prediction_probas(self.tree_clf_grid_bag_new_man_add, self.X_man_add_train, self.y_train, self.X_man_add_test, self.y_test, self.y_pred_proba_man_add, self.y_train_CV_pred_proba_man_add, self.man_add, 'man_add')    
    prediction_probas(self.tree_clf_grid_bag_new_transform, self.X_transform_train, self.y_train, self.X_transform_test, self.y_test, self.y_pred_proba_transform, self.y_train_CV_pred_proba_transform, self.transform, 'transform')    
    prediction_probas(self.tree_clf_grid_bag_new_prot_screen_trans, self.X_prot_screen_trans_train, self.y_train, self.X_prot_screen_trans_test, self.y_test, self.y_pred_proba_prot_screen_trans, self.y_train_CV_pred_proba_prot_screen_trans, self.prot_screen_trans, 'prot_screen_trans')  
    ...
    scoring_all(self.tree_clf_grid_bag_new_database, self.X_database_train, self.y_train, self.X_database_test, self.y_test, self.database)
    scoring_all(self.tree_clf_grid_bag_new_man_add, self.X_man_add_train, self.y_train, self.X_man_add_test, self.y_test, self.man_add)
    scoring_all(self.tree_clf_grid_bag_new_transform, self.X_transform_train, self.y_train, self.X_transform_test, self.y_test, self.transform)
    scoring_all(self.tree_clf_grid_bag_new_prot_screen_trans, self.X_prot_screen_trans_train, self.y_train, self.X_prot_screen_trans_test, self.y_test, self.prot_screen_trans)
...
def run():
  ...
  decision_tree_bag_grid_search = DecisionTreeBagGridSearch(metrix, database, man_add, transform, prot_screen_trans)
