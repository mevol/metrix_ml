...
class CorrelCoeffAnalysis(object):
    ...
    p-values; create a scatter matrix'''
    ...
    def corrcoef_loop(X_train):
        if X_train.isnull().any().any() == True:
            X_train = X_train.dropna(axis=1)
        X_train = X_train.drop(labels='pdb_id_id', axis=1)
        #print(X_train)
        attr = list(X_train)
        #print(attr)
        rows = len(attr)
        ...
        for i in range(rows):
            for j in range(i+1, rows):
                ...
                p[i, j] = p[j, i] = p_
        return r, p
    r,p = corrcoef_loop(metrix)
    
    def write_corr(r, p, X_train):
        with open(os.path.join(METRIX_PATH, 'linear_corr_pvalues_X_database_train_work.csv'), 'a') as csv_file:
            #if X_train.isnull().any().any() == True:
            #    X_train = X_train.dropna(axis=1)
            #X_train = X_train.drop(labels='pdb_id_id', axis=1)
            #print(X_train)
