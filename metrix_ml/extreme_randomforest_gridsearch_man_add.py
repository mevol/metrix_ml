...
def make_output_folder(outdir):
  ...
  result = []
  for name in names:
    name = os.path.join(outdir, 'extreme_randomforest_gridsearch', name)
...
class ExtremeRandomForestGridSearch(object):
  ...
  def prepare_metrix_data(self):
    ...
    ******
    ...
    with open(os.path.join(self.man_add_only, 'extreme_randomforest_gridsearch.txt'), 'a') as text_file:
...
#    X_man_add_train, X_man_add_test, y_train, y_test = train_test_split(self.X_man_add, y, test_size=0.2, random_state=42, stratify=y)
  ...
  def grid_search(self):
    ...
    extra_clf_grid = ExtraTreesClassifier(random_state=42)

    with open(os.path.join(self.man_add_only, 'extreme_randomforest_gridsearch.txt'), 'a') as text_file:
      text_file.write('Created random forest: extra_clf_grid \n')
    ...
    param_grid = {"criterion": ["gini", "entropy"],#metric to judge reduction of impurity
                 ...
                 'max_features': [1, 2, 4, 8, 16, 21, 29],#max number of features when splitting
    ...
    grid_search = GridSearchCV(extra_clf_grid, param_grid, cv=10, scoring='accuracy')
  ...
  def forest_best_params(self):
    ...
    self.extra_clf_grid_new_man_add = ExtraTreesClassifier(**self.best_params_man_add, random_state=42)
    self.extra_clf_grid_new_man_add.fit(self.X_man_add_train, self.y_train)
    ...
    feature_importances_pandas(self.extra_clf_grid_new_man_add, self.X_man_add_train, 'man_add', self.man_add_only)

    def write_pickle(forest, directory, name):
      datestring = datetime.strftime(datetime.now(), '%Y%m%d_%H%M')
      joblib.dump(forest, os.path.join(directory,'best_forest_grid_'+name+datestring+'.pkl'))
      with open(os.path.join(directory, 'extreme_randomforest_gridsearch.txt'), 'a') as text_file:
        text_file.write('Created new random forest "extra_clf_grid_new_%s" using best parameters \n' %name)
        text_file.write('Creating pickle file for best forest as best_forest_grid_%s.pkl \n' %name)
    
    write_pickle(self.extra_clf_grid_new_man_add, self.man_add_only, 'man_add')
    ...
    visualise_tree(self.extra_clf_grid_new_man_add, self.man_add_only, self.X_man_add_train.columns, 'man_add')
    ...
    basic_stats(self.extra_clf_grid_new_man_add, self.X_man_add_train, self.man_add_only)
  ...
  def predict(self):
    ...
    #self.y_pred_man_add = self.extra_clf_grid_new_man_add.predict(self.X_man_add_test)
    self.y_pred_man_add = self.extra_clf_grid_new_man_add.predict(self.X_man_add_test)
    self.y_pred_proba_man_add = self.extra_clf_grid_new_man_add.predict_proba(self.X_man_add_test)
    with open(os.path.join(self.man_add_only, 'extreme_randomforest_gridsearch.txt'), 'a') as text_file:
      text_file.write('Saving predictions and probabilities for X_man_add_test in y_pred_man_add and probabilities in y_pred_proba_man_add \n')

    #alternative way to not have to use the test set
    self.y_train_CV_pred_man_add = cross_val_predict(self.extra_clf_grid_new_man_add, self.X_man_add_train, self.y_train, cv=10)
    self.y_train_CV_pred_proba_man_add = cross_val_predict(self.extra_clf_grid_new_man_add, self.X_man_add_train, self.y_train, cv=10, method='predict_proba')
    with open(os.path.join(self.man_add_only, 'extreme_randomforest_gridsearch.txt'), 'a') as text_file:
  ...
  def analysis(self):
    ...
    prediction_probas(self.extra_clf_grid_new_man_add, self.X_man_add_train, self.y_train, self.X_man_add_test, self.y_test, self.y_pred_proba_man_add, self.y_train_CV_pred_proba_man_add, self.man_add_only, 'man_add')    
...
def run():
  ...
  extreme_random_forest_grid_search = ExtremeRandomForestGridSearch(metrix, man_add_only, bbbb)
