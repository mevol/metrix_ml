...
class DecisionTreeBagRandomSearch(object):
  ...
  def prepare_metrix_data(self):
    ...
    ******
    ...
    metrix_database_normal = metrix_database[['RpimdiffI', 'anomalousmulti', 'lowreslimit', 'anomalousslope', 'totalunique']].copy()
    
    metrix_database_stratified = metrix_database[['RpimdiffI', 'IoverSigma', 'totalunique', 'wilsonbfactor', 'diffF']].copy()
...
#    X_database_train, X_database_test, y_train, y_test = train_test_split(self.X_database, y, test_size=0.2, random_state=42, stratify=y)
  ...
  def tree_best_params(self):
    ...
    feature_importances_pandas(self.tree_clf_rand_bag_new_database, self.X_database_train, 'database', self.database_features)
    ...
    write_pickle(self.tree_clf_rand_bag_new_database, self.database_features, 'database')
    ...
    visualise_tree(self.tree_clf_rand_bag_new_database, self.database_features, self.X_database_train.columns, 'database')
    ...
    basic_stats(self.tree_clf_rand_bag_new_database, self.X_database_train, self.database_features)
  ...
  def analysis(self):
    ...
    prediction_probas(self.tree_clf_rand_bag_new_database, self.X_database_train, self.y_train, self.X_database_test, self.y_test, self.y_pred_proba_database, self.y_train_CV_pred_proba_database, self.database_features, 'database')    
...
def run():
  ...
  decision_tree_bag_rand_search = DecisionTreeBagRandomSearch(metrix, database_features, bbbb)
