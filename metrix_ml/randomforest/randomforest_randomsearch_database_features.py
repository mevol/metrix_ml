...
class RandomForestRandSearch(object):
  ...
  def prepare_metrix_data(self):
    ...
    ******
    ...
    with open(os.path.join(self.database_features, 'randomforest_randomsearch.txt'), 'a') as text_file:
      text_file.write('Preparing input data as metrix_database with following attributes %s \n' %(attr_database))
    
    metrix_database_normal = metrix_database[['totalobservations', 'RpimdiffI', 'lowreslimit', 'diffF', 'IoverSigma']].copy()
    
    metrix_database_stratified = metrix_database[['totalobservations', 'anomalousCC', 'diffF', 'totalunique', 'anomalousmulti']].copy()
...
#stratified split of samples
  ...
  def forest_best_params(self):
    ...
    feature_importances_pandas(self.forest_clf_rand_new_database, self.X_database_train, 'database', self.database_features)
    ...
    write_pickle(self.forest_clf_rand_new_database, self.database_features, 'database')
    ...
    visualise_tree(self.forest_clf_rand_new_database, self.database_features, self.X_database_train.columns, 'database')
    ...
    basic_stats(self.forest_clf_rand_new_database, self.X_database_train, self.database_features)
  ...
  def analysis(self):
    ...
    prediction_probas(self.forest_clf_rand_new_database, self.X_database_train, self.y_train, self.X_database_test, self.y_test, self.y_pred_proba_database, self.y_train_CV_pred_proba_database, self.database_features, 'database')    
...
def run():
  ...
  random_forest_rand_search = RandomForestRandSearch(metrix, database_features, bbbb)
