...
class RandomForestGridSearch(object):
  ...
  def prepare_metrix_data(self):
    ...
    ******
    ...
    with open(os.path.join(self.prot_screen_trans, 'randomforest_gridsearch.txt'), 'a') as text_file:
...
#    X_prot_screen_trans_train, X_prot_screen_trans_test, y_train, y_test = train_test_split(self.X_prot_screen_trans, y, test_size=0.2, random_state=42, stratify=y)
  ...
  def grid_search(self):
    ...
    param_grid = {"criterion": ["gini", "entropy"],#metric to judge reduction of impurity
                 'n_estimators': [10, 50, 100, 500],#number of trees in forest
    ...
    grid_search = GridSearchCV(forest_clf_grid, param_grid, cv=10, scoring='accuracy')
    ...
    feature_importances_man_add = grid_search_man_add.best_estimator_.feature_importances_
    ...
    feature_importances_transform = grid_search_transform.best_estimator_.feature_importances_
  ...
  def forest_best_params(self):
    ...
    self.forest_clf_grid_new_prot_screen_trans = RandomForestClassifier(**self.best_params_prot_screen_trans, random_state=42)
    self.forest_clf_grid_new_prot_screen_trans.fit(self.X_prot_screen_trans_train, self.y_train)
    ...
    feature_importances_pandas(self.forest_clf_grid_new_database, self.X_database_train, 'database', self.database)
    feature_importances_pandas(self.forest_clf_grid_new_man_add, self.X_man_add_train, 'man_add', self.man_add)
    feature_importances_pandas(self.forest_clf_grid_new_transform, self.X_transform_train, 'transform', self.transform)
    feature_importances_pandas(self.forest_clf_grid_new_prot_screen_trans, self.X_prot_screen_trans_train, 'prot_screen_trans', self.prot_screen_trans)

    def write_pickle(forest, directory, name):
      datestring = datetime.strftime(datetime.now(), '%Y%m%d_%H%M')
      joblib.dump(forest, os.path.join(directory,'best_forest_grid_'+name+datestring+'.pkl'))
      with open(os.path.join(directory, 'randomforest_gridsearch.txt'), 'a') as text_file:
        text_file.write('Created new random forest "forest_clf_grid_new_%s" using best parameters \n' %name)
        text_file.write('Creating pickle file for best forest as best_forest_grid_%s.pkl \n' %name)
    
    write_pickle(self.forest_clf_grid_new_database, self.database, 'database')
    write_pickle(self.forest_clf_grid_new_man_add, self.man_add, 'man_add')
    write_pickle(self.forest_clf_grid_new_transform, self.transform, 'transform')
    write_pickle(self.forest_clf_grid_new_prot_screen_trans, self.prot_screen_trans, 'prot_screen_trans')
    ...
    def visualise_tree(tree_forest, directory, columns, name):
      ...
      i_tree = 0
      for tree in trees:
        with open(os.path.join(directory,'tree_clf_grid_new_'+name+datestring+str(i_tree)+'.dot'), 'w') as f:
          export_graphviz(tree, out_file=f, feature_names=columns, rounded=True, filled=True)
          f.close()
        dotfile = os.path.join(directory, 'tree_clf_grid_new_'+name+datestring+str(i_tree)+'.dot')
        pngfile = os.path.join(directory, 'tree_clf_grid_new_'+name+datestring+str(i_tree)+'.png')
        command = ["dot", "-Tpng", dotfile, "-o", pngfile]
        subprocess.check_call(command)
        i_tree = i_tree + 1

      with open(os.path.join(directory, 'randomforest_gridsearch.txt'), 'a') as text_file:
        text_file.write('Writing DOTfile and convert to PNG for "tree_clf_grid_new_%s" \n' %name)
        text_file.write('DOT filename: tree_clf_grid_new_%s.dot \n' %name)
        text_file.write('PNG filename: tree_clf_grid_new_%s.png \n' %name)

    
    visualise_tree(self.forest_clf_grid_new_database, self.database, self.X_database_train.columns, 'database')
    visualise_tree(self.forest_clf_grid_new_man_add, self.man_add, self.X_man_add_train.columns, 'man_add')
    visualise_tree(self.forest_clf_grid_new_transform, self.transform, self.X_transform_train.columns, 'transform')
    visualise_tree(self.forest_clf_grid_new_prot_screen_trans, self.prot_screen_trans, self.X_prot_screen_trans_train.columns, 'prot_screen_trans')
    ...
    basic_stats(self.forest_clf_grid_new_database, self.X_database_train, self.database)
    basic_stats(self.forest_clf_grid_new_man_add, self.X_man_add_train, self.man_add)
    basic_stats(self.forest_clf_grid_new_transform, self.X_transform_train, self.transform)
    basic_stats(self.forest_clf_grid_new_prot_screen_trans, self.X_prot_screen_trans_train, self.prot_screen_trans)
  ...
  def predict(self):
    ...
    #self.y_pred_database = self.forest_clf_grid_new_database.predict(self.X_database_test)
    self.y_pred_database = self.forest_clf_grid_new_database.predict(self.X_database_test)
    self.y_pred_proba_database = self.forest_clf_grid_new_database.predict_proba(self.X_database_test)
    with open(os.path.join(self.database, 'randomforest_gridsearch.txt'), 'a') as text_file:
    ...
    #self.y_pred_man_add = self.forest_clf_grid_new_man_add.predict(self.X_man_add_test)
    self.y_pred_man_add = self.forest_clf_grid_new_man_add.predict(self.X_man_add_test)
    self.y_pred_proba_man_add = self.forest_clf_grid_new_man_add.predict_proba(self.X_man_add_test)
    with open(os.path.join(self.man_add, 'randomforest_gridsearch.txt'), 'a') as text_file:
    ...
    #self.y_pred_transform = self.forest_clf_grid_new_transform.predict(self.X_transform_test)
    self.y_pred_transform = self.forest_clf_grid_new_transform.predict(self.X_transform_test)
    self.y_pred_proba_transform = self.forest_clf_grid_new_transform.predict_proba(self.X_transform_test)
    with open(os.path.join(self.transform, 'randomforest_gridsearch.txt'), 'a') as text_file:
    ...
    #self.y_pred_prot_screen_trans = self.forest_clf_grid_new_prot_screen_trans.predict(self.X_prot_screen_trans_test)
    self.y_pred_prot_screen_trans = self.forest_clf_grid_new_prot_screen_trans.predict(self.X_prot_screen_trans_test)
    self.y_pred_proba_prot_screen_trans = self.forest_clf_grid_new_prot_screen_trans.predict_proba(self.X_prot_screen_trans_test)
    with open(os.path.join(self.prot_screen_trans, 'randomforest_gridsearch.txt'), 'a') as text_file:
    ...
    self.y_train_CV_pred_database = cross_val_predict(self.forest_clf_grid_new_database, self.X_database_train, self.y_train, cv=10)
    self.y_train_CV_pred_proba_database = cross_val_predict(self.forest_clf_grid_new_database, self.X_database_train, self.y_train, cv=10, method='predict_proba')
    with open(os.path.join(self.database, 'randomforest_gridsearch.txt'), 'a') as text_file:
      text_file.write('Saving predictions and probabilities for X_database_train with 10-fold CV in y_train_pred_database \n')
    self.y_train_CV_pred_man_add = cross_val_predict(self.forest_clf_grid_new_man_add, self.X_man_add_train, self.y_train, cv=10)
    self.y_train_CV_pred_proba_man_add = cross_val_predict(self.forest_clf_grid_new_man_add, self.X_man_add_train, self.y_train, cv=10, method='predict_proba')
    with open(os.path.join(self.man_add, 'randomforest_gridsearch.txt'), 'a') as text_file:
      text_file.write('Saving predictions and probabilities for X_man_add_train with 10-fold CV in y_train_pred_man_add \n')
    self.y_train_CV_pred_transform = cross_val_predict(self.forest_clf_grid_new_transform, self.X_transform_train, self.y_train, cv=10)
    self.y_train_CV_pred_proba_transform = cross_val_predict(self.forest_clf_grid_new_transform, self.X_transform_train, self.y_train, cv=10, method='predict_proba')
    with open(os.path.join(self.transform, 'randomforest_gridsearch.txt'), 'a') as text_file:
      text_file.write('Saving predictions and probabilities for X_transform_train with 10-fold CV in y_train_pred_transform \n')
    self.y_train_CV_pred_prot_screen_trans = cross_val_predict(self.forest_clf_grid_new_prot_screen_trans, self.X_prot_screen_trans_train, self.y_train, cv=10)
    self.y_train_CV_pred_proba_prot_screen_trans = cross_val_predict(self.forest_clf_grid_new_prot_screen_trans, self.X_prot_screen_trans_train, self.y_train, cv=10, method='predict_proba')
    with open(os.path.join(self.prot_screen_trans, 'randomforest_gridsearch.txt'), 'a') as text_file:
  ...
  def analysis(self):
    ...
    def conf_mat(y_test, y_train, y_pred, y_train_pred, directory):
      ...
      def draw_conf_mat(matrix, directory, name):
        ...
        plt.ylabel('True')
        plt.savefig(os.path.join(directory, 'confusion_matrix_forest_grid_'+name+datestring+'.png'))
        plt.close()
...
#      precision_recall(self.y_train, y_train_CV_pred_proba, 'train_CV_', directory)
    ...
    prediction_probas(self.forest_clf_grid_new_database, self.X_database_train, self.y_train, self.X_database_test, self.y_test, self.y_pred_proba_database, self.y_train_CV_pred_proba_database, self.database, 'database')    
    prediction_probas(self.forest_clf_grid_new_man_add, self.X_man_add_train, self.y_train, self.X_man_add_test, self.y_test, self.y_pred_proba_man_add, self.y_train_CV_pred_proba_man_add, self.man_add, 'man_add')    
    prediction_probas(self.forest_clf_grid_new_transform, self.X_transform_train, self.y_train, self.X_transform_test, self.y_test, self.y_pred_proba_transform, self.y_train_CV_pred_proba_transform, self.transform, 'transform')    
    prediction_probas(self.forest_clf_grid_new_prot_screen_trans, self.X_prot_screen_trans_train, self.y_train, self.X_prot_screen_trans_test, self.y_test, self.y_pred_proba_prot_screen_trans, self.y_train_CV_pred_proba_prot_screen_trans, self.prot_screen_trans, 'prot_screen_trans')  
    ...
    scoring_all(self.forest_clf_grid_new_database, self.X_database_train, self.y_train, self.X_database_test, self.y_test, self.database)
    scoring_all(self.forest_clf_grid_new_man_add, self.X_man_add_train, self.y_train, self.X_man_add_test, self.y_test, self.man_add)
    scoring_all(self.forest_clf_grid_new_transform, self.X_transform_train, self.y_train, self.X_transform_test, self.y_test, self.transform)
    scoring_all(self.forest_clf_grid_new_prot_screen_trans, self.X_prot_screen_trans_train, self.y_train, self.X_prot_screen_trans_test, self.y_test, self.prot_screen_trans)
...
def run():
  ...
  random_forest_grid_search = RandomForestGridSearch(metrix, database, man_add, transform, prot_screen_trans)
