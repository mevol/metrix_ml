...
class RandomForestGridSearch(object):
  ...
  def prepare_metrix_data(self):
    ...
    ******
    ...
    with open(os.path.join(self.transform_features, 'randomforest_gridsearch.txt'), 'a') as text_file:
    ...
    metrix_transform_normal = metrix_transform[['RpimdiffI', 'MW_chain/No_atom_chain', 'MW_ASU/sites_ASU', 'Matth_coeff', 'totalobservations']].copy()
    
    metrix_transform_stratified = metrix_transform[['MW_chain/No_atom_chain', 'Matth_coeff', 'totalobservations', 'MW_ASU/sites_ASU']].copy()
    
    self.X_transform = metrix_transform_normal
...
#    X_transform_train, X_transform_test, y_train, y_test = train_test_split(self.X_transform, y, test_size=0.2, random_state=42, stratify=y)
  ...
  def grid_search(self):
    ...
    param_grid = {"criterion": ["gini", "entropy"],#metric to judge reduction of impurity
                 ...
                 'max_features': [1, 2, 4, 5],#max number of features when splitting
  ...
  def forest_best_params(self):
    ...
    feature_importances_pandas(self.forest_clf_grid_new_transform, self.X_transform_train, 'transform', self.transform_features)
    ...
    write_pickle(self.forest_clf_grid_new_transform, self.transform_features, 'transform')
    ...
    visualise_tree(self.forest_clf_grid_new_transform, self.transform_features, self.X_transform_train.columns, 'transform')
    ...
    basic_stats(self.forest_clf_grid_new_transform, self.X_transform_train, self.transform_features)
  ...
  def analysis(self):
    ...
    prediction_probas(self.forest_clf_grid_new_transform, self.X_transform_train, self.y_train, self.X_transform_test, self.y_test, self.y_pred_proba_transform, self.y_train_CV_pred_proba_transform, self.transform_features, 'transform')    
...
def run():
  ...
  random_forest_grid_search = RandomForestGridSearch(metrix, transform_features, bbbb)
