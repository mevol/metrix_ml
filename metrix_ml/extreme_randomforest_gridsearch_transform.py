...
class ExtremeRandomForestGridSearch(object):
  ...
  def prepare_metrix_data(self):
    ...
    ******
    ...
    with open(os.path.join(self.transform_only, 'extreme_randomforest_gridsearch.txt'), 'a') as text_file:
...
#    X_transform_train, X_transform_test, y_train, y_test = train_test_split(self.X_transform, y, test_size=0.2, random_state=42, stratify=y)
  ...
  def grid_search(self):
    ...
    param_grid = {"criterion": ["gini", "entropy"],#metric to judge reduction of impurity
                 ...
                 'max_features': [1, 2, 4, 8, 16, 21, 29, 39],#max number of features when splitting
  ...
  def forest_best_params(self):
    ...
    self.extra_clf_grid_new_transform = ExtraTreesClassifier(**self.best_params_transform, random_state=42)
    self.extra_clf_grid_new_transform.fit(self.X_transform_train, self.y_train)
    ...
    feature_importances_pandas(self.extra_clf_grid_new_transform, self.X_transform_train, 'transform', self.transform_only)
    ...
    write_pickle(self.extra_clf_grid_new_transform, self.transform_only, 'transform')
    ...
    visualise_tree(self.extra_clf_grid_new_transform, self.transform_only, self.X_transform_train.columns, 'transform')
    ...
    basic_stats(self.extra_clf_grid_new_transform, self.X_transform_train, self.transform_only)
  ...
  def predict(self):
    ...
    #self.y_pred_transform = self.extra_clf_grid_new_transform.predict(self.X_transform_test)
    self.y_pred_transform = self.extra_clf_grid_new_transform.predict(self.X_transform_test)
    self.y_pred_proba_transform = self.extra_clf_grid_new_transform.predict_proba(self.X_transform_test)
    with open(os.path.join(self.transform_only, 'extreme_randomforest_gridsearch.txt'), 'a') as text_file:
    ...
    self.y_train_CV_pred_transform = cross_val_predict(self.extra_clf_grid_new_transform, self.X_transform_train, self.y_train, cv=10)
    self.y_train_CV_pred_proba_transform = cross_val_predict(self.extra_clf_grid_new_transform, self.X_transform_train, self.y_train, cv=10, method='predict_proba')
    with open(os.path.join(self.transform_only, 'extreme_randomforest_gridsearch.txt'), 'a') as text_file:
  ...
  def analysis(self):
    ...
    prediction_probas(self.extra_clf_grid_new_transform, self.X_transform_train, self.y_train, self.X_transform_test, self.y_test, self.y_pred_proba_transform, self.y_train_CV_pred_proba_transform, self.transform_only, 'transform')    
...
def run():
  ...
  extreme_random_forest_grid_search = ExtremeRandomForestGridSearch(metrix, transform_only, bbbb)
