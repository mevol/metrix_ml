...
class RandomForestGridSearch(object):
  ...
  def forest_best_params(self):
    ...
    feature_importances(self.forest_clf_grid_new_database, self.X_database_train, self.database)
    feature_importances(self.forest_clf_grid_new_man_add, self.X_man_add_train, self.man_add)
    feature_importances(self.forest_clf_grid_new_transform, self.X_transform_train, self.transform)
    feature_importances(self.forest_clf_grid_new_prot_screen_trans, self.X_prot_screen_trans_train, self.prot_screen_trans)
  ...
  def predict(self):
    ...
    self.y_pred_class_database = self.forest_clf_grid_new_database.predict(self.X_database_test)
    with open(os.path.join(self.database, 'randomforest_gridsearch.txt'), 'a') as text_file:
      text_file.write('Saving predictions for X_database_test in y_pred_class_database \n')
    self.y_pred_class_man_add = self.forest_clf_grid_new_man_add.predict(self.X_man_add_test)
    with open(os.path.join(self.man_add, 'randomforest_gridsearch.txt'), 'a') as text_file:
      text_file.write('Saving predictions for X_man_add_test in y_pred_class_man_add \n')
    self.y_pred_class_transform = self.forest_clf_grid_new_transform.predict(self.X_transform_test)
    with open(os.path.join(self.transform, 'randomforest_gridsearch.txt'), 'a') as text_file:
      text_file.write('Saving predictions for X_transform_test in y_pred_class_transform \n')
    self.y_pred_class_prot_screen_trans = self.forest_clf_grid_new_prot_screen_trans.predict(self.X_prot_screen_trans_test)
    with open(os.path.join(self.prot_screen_trans, 'randomforest_gridsearch.txt'), 'a') as text_file:
    ...
    self.y_train_pred_database = cross_val_predict(self.forest_clf_grid_new_database, self.X_database_train, self.y_train, cv=10)
    with open(os.path.join(self.database, 'randomforest_gridsearch.txt'), 'a') as text_file:
      text_file.write('Saving predictions for X_database_train with 10-fold CV in y_train_pred_database \n')
    self.y_train_pred_man_add = cross_val_predict(self.forest_clf_grid_new_man_add, self.X_man_add_train, self.y_train, cv=10)
    with open(os.path.join(self.man_add, 'randomforest_gridsearch.txt'), 'a') as text_file:
      text_file.write('Saving predictions for X_man_add_train with 10-fold CV in y_train_pred_man_add \n')
    self.y_train_pred_transform = cross_val_predict(self.forest_clf_grid_new_transform, self.X_transform_train, self.y_train, cv=10)
    with open(os.path.join(self.transform, 'randomforest_gridsearch.txt'), 'a') as text_file:
      text_file.write('Saving predictions for X_transform_train with 10-fold CV in y_train_pred_transform \n')
    self.y_train_pred_prot_screen_trans = cross_val_predict(self.forest_clf_grid_new_prot_screen_trans, self.X_prot_screen_trans_train, self.y_train, cv=10)
    with open(os.path.join(self.prot_screen_trans, 'randomforest_gridsearch.txt'), 'a') as text_file:
  ...
  def analysis(self):
    ...
    def prediction_probas(forest, X_train, y_train, X_test, y_test, directory, kind): 
      ...
      def plot_hist_pred_proba(y_pred_proba, name, directory):
        ...
        plt.savefig(os.path.join(directory, 'hist_pred_proba_forest_grid_'+name+datestring+'.png'))
        plt.close()
      ...
      def plot_precision_recall_vs_threshold(precisions, recalls, thresholds_forest, name, directory):
        ...
        plt.ylim([0,1])
        plt.savefig(os.path.join(directory, 'Precision_Recall_forest_grid_'+name+datestring+'.png'))
        plt.close()
      ...
      def plot_roc_curve(fpr, tpr, name, label=None):
        ...
        plt.grid(True)
        plt.savefig(os.path.join(directory, 'ROC_curve_forest_grid_'+name+datestring+'.png'))
        plt.close()
    ...
    prediction_probas(self.forest_clf_grid_new_database, self.X_database_train, self.y_train, self.X_database_test, self.y_test, self.database, 'database')
    prediction_probas(self.forest_clf_grid_new_man_add, self.X_man_add_train, self.y_train, self.X_man_add_test, self.y_test, self.man_add, 'man_add')
    prediction_probas(self.forest_clf_grid_new_transform, self.X_transform_train, self.y_train, self.X_transform_test, self.y_test, self.transform, 'transform')
    prediction_probas(self.forest_clf_grid_new_prot_screen_trans, self.X_prot_screen_trans_train, self.y_train, self.X_prot_screen_trans_test, self.y_test, self.prot_screen_trans, 'prot_screen_trans')
