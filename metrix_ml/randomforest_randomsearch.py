...
def make_output_folder(outdir):
  out_folder = os.path.join(outdir, 'randomforest_randomsearch')
...
class RandomForestRandSearch(object):
  ...
  def forest_best_params(self):
    ...
    joblib.dump(self.forest_clf_rand_new, os.path.join(self.out_folder,'best_forest_rand_search.pkl'))
    with open(os.path.join(self.out_folder, 'randomforest_randomsearch.txt'), 'a') as text_file:
    ...
    i_tree = 0
    for tree in trees:
      with open(os.path.join(self.out_folder,'forest_clf_rand_new_tree' + str(i_tree) + '.dot'), 'w') as f:
        export_graphviz(tree, out_file=f, feature_names=self.X_transform_train.columns,
                   rounded=True, filled=True)
        f.close()
      dotfile = os.path.join(self.out_folder, 'forest_clf_rand_new_tree' + str(i_tree) + '.dot')
      pngfile = os.path.join(self.out_folder, 'forest_clf_rand_new_tree' + str(i_tree) + '.png')
...
#    self.y_scores=self.forest_clf_rand_new.predict_proba(self.X_transform_train)#train set
    ...
    def plot_hist_pred_proba(y_pred_proba, name):
      ...
      plt.savefig(os.path.join(self.out_folder, 'hist_pred_proba_forest_rand_'+name+datestring+'.png'))
      plt.close()
    ...
    def plot_precision_recall_vs_threshold(precisions, recalls, thresholds_forest, name):
      ...
      plt.ylim([0,1])
      plt.savefig(os.path.join(self.out_folder, 'Precision_Recall_forest_rand_'+name+datestring+'.png'))
      plt.close()
    ...
    def plot_roc_curve(fpr, tpr, name, label=None):
      ...
      plt.grid(True)
      plt.savefig(os.path.join(self.out_folder, 'ROC_curve_forest_rand_'+name+datestring+'.png'))
      plt.close()
...
def run():
  ...
  random_forest_rand_search = RandomForestRandSearch(metrix, outdir)
