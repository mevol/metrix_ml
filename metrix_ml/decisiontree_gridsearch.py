...
def parse_command_line():
  ...
  parser = argparse.ArgumentParser(description='Decision tree grid search')
...
def make_output_folder(outdir):
  ...
  result = []
  for name in names:
    name = os.path.join(outdir, 'decisiontree_gridsearch', name)
...
#stratified split of samples
  ...
  def grid_search(self):
    ...
    grid_search = GridSearchCV(tree_clf_grid, param_grid, cv=10,
                              scoring='accuracy')
  ...
  def tree_best_params(self):
    ...
    self.tree_clf_grid_new_database = DecisionTreeClassifier(**self.best_params_database, random_state=42)
    self.tree_clf_grid_new_database.fit(self.X_database_train, self.y_train)

    self.tree_clf_grid_new_man_add = DecisionTreeClassifier(**self.best_params_man_add, random_state=42)
    self.tree_clf_grid_new_man_add.fit(self.X_man_add_train, self.y_train)

    self.tree_clf_grid_new_transform = DecisionTreeClassifier(**self.best_params_transform, random_state=42)
    self.tree_clf_grid_new_transform.fit(self.X_transform_train, self.y_train)
    ...
    def write_pickle(tree, directory, name):
      datestring = datetime.strftime(datetime.now(), '%Y%m%d_%H%M')
      joblib.dump(tree, os.path.join(directory,'best_tree_grid_'+name+datestring+'.pkl'))
      with open(os.path.join(directory, 'decisiontree_gridsearch.txt'), 'a') as text_file:
        text_file.write('Created new decision tree "tree_clf_grid_new_%s" using best parameters \n' %name)
        text_file.write('Creating pickle file for best tree as best_tree_grid_%s.pkl \n' %name)
    
    write_pickle(self.tree_clf_grid_new_database, self.database, 'database')
    write_pickle(self.tree_clf_grid_new_man_add, self.man_add, 'man_add')
    write_pickle(self.tree_clf_grid_new_transform, self.transform, 'transform')
    write_pickle(self.tree_clf_grid_new_prot_screen_trans, self.prot_screen_trans, 'prot_screen_trans')

    def visualise_tree(tree, directory, columns, name):
      datestring = datetime.strftime(datetime.now(), '%Y%m%d_%H%M')
      dotfile = os.path.join(directory, 'tree_clf_grid_new_'+name+datestring+'.dot')
      pngfile = os.path.join(directory, 'tree_clf_grid_new_'+name+datestring+'.png')
    ...
    visualise_tree(self.tree_clf_grid_new_database, self.database, self.X_database_train.columns, 'database')
    visualise_tree(self.tree_clf_grid_new_man_add, self.man_add, self.X_man_add_train.columns, 'man_add')
    visualise_tree(self.tree_clf_grid_new_transform, self.transform, self.X_transform_train.columns, 'transform')
    visualise_tree(self.tree_clf_grid_new_prot_screen_trans, self.prot_screen_trans, self.X_prot_screen_trans_train.columns, 'prot_screen_trans')
    ...
    basic_stats(self.tree_clf_grid_new_database, self.X_database_train, self.database)
    basic_stats(self.tree_clf_grid_new_man_add, self.X_man_add_train, self.man_add)
    basic_stats(self.tree_clf_grid_new_transform, self.X_transform_train, self.transform)
    basic_stats(self.tree_clf_grid_new_prot_screen_trans, self.X_prot_screen_trans_train, self.prot_screen_trans)
  ...
  def analysis(self):
    ...
    def conf_mat(y_test, y_train, y_pred, y_train_pred, directory):
      ...
      def draw_conf_mat(matrix, directory, name):
        ...
        plt.ylabel('True')
        plt.savefig(os.path.join(directory, 'confusion_matrix_tree_grid_'+name+datestring+'.png'))
        plt.close()
...
#      precision_recall(self.y_train, y_train_CV_pred_proba, 'train_CV_', directory)
    ...
    scoring_all(self.tree_clf_grid_new_database, self.X_database_train, self.y_train, self.X_database_test, self.y_test, self.database)
    scoring_all(self.tree_clf_grid_new_man_add, self.X_man_add_train, self.y_train, self.X_man_add_test, self.y_test, self.man_add)
    scoring_all(self.tree_clf_grid_new_transform, self.X_transform_train, self.y_train, self.X_transform_test, self.y_test, self.transform)
    scoring_all(self.tree_clf_grid_new_prot_screen_trans, self.X_prot_screen_trans_train, self.y_train, self.X_prot_screen_trans_test, self.y_test, self.prot_screen_trans)
...
def run():
  ...
  decision_tree_grid_search = DecisionTreeGridSearch(metrix, database, man_add, transform, prot_screen_trans)
