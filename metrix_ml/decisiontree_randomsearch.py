...
class DecisionTreeRandomSearch(object):
  ...
  def tree_best_params(self):
    ...
    joblib.dump(self.tree_clf_rand_new, os.path.join(self.out_folder,'best_tree_rand_search.pkl'))
    with open(os.path.join(self.out_folder, 'decisiontree_randomsearch.txt'), 'a') as text_file:
...
def run():
  ...
  decision_tree_rand_search = DecisionTreeRandomSearch(metrix, outdir)
