...
class DecisionTreeGridSearchManAdd(object):
  ...
  def prepare_metrix_data(self):
    ...
    ******
    ...
    attr_man_add_normal = ['totalobservations', 'diffF','Matth_coeff', 'sites_ASU']

    attr_man_add_stratified = ['anomalousCC', 'totalobservations', 'RpimdiffI', 'Matth_coeff', 'MW_chain']
                    
    metrix_man_add = self.metrix[attr_man_add_stratified]

    with open(os.path.join(self.man_add_features, 'decisiontree_gridsearch.txt'), 'a') as text_file:
      text_file.write('Preparing input data as metrix_man_add with following attributes %s \n' %(attr_man_add_stratified))
...
#stratified split of samples
  ...
  def grid_search(self):
    ...
    param_grid = {"criterion": ["gini", "entropy"],#metric to judge reduction of impurity
                  'max_features': [1, 2, 5],#max number of features when splitting
...
#    feature_importances_pandas(self.tree_clf_grid_new_database, self.X_database_train, 'database', self.database)      
    ...
    plot_features(self.tree_clf_grid_new_man_add, self.X_man_add_train.columns, self.man_add_features)
    ...
    write_pickle(self.tree_clf_grid_new_man_add, self.man_add_features, 'man_add')
    ...
    visualise_tree(self.tree_clf_grid_new_man_add, self.man_add_features, self.X_man_add_train.columns, 'man_add')
    ...
    basic_stats(self.tree_clf_grid_new_man_add, self.X_man_add_train, self.man_add_features)
  ...
  def analysis(self):
    ...
    prediction_probas(self.tree_clf_grid_new_man_add, self.X_man_add_train, self.y_train, self.X_man_add_test, self.y_test, self.y_pred_proba_man_add, self.y_train_CV_pred_proba_man_add, self.man_add_features, 'man_add')    
...
def run():
  ...
  decision_tree_grid_search_man_add = DecisionTreeGridSearchManAdd(metrix, man_add_features, bbbb)
