...
#    plot_features(self.tree_prot_screen_trans, self.X_prot_screen_trans_train.columns, self.prot_screen_trans)
  ...
  def tree_best_params(self):
    ...
    feature_importances_pandas(self.tree_clf_rand_bag_new_database, self.X_database_train, 'database', self.database)
    feature_importances_pandas(self.tree_clf_rand_bag_new_man_add, self.X_man_add_train, 'man_add', self.man_add)
    feature_importances_pandas(self.tree_clf_rand_bag_new_transform, self.X_transform_train, 'transform', self.transform)
    feature_importances_pandas(self.tree_clf_rand_bag_new_prot_screen_trans, self.X_prot_screen_trans_train, 'prot_screen_trans', self.prot_screen_trans)
  ...
  def predict(self):
    ...
    #self.y_pred_database = self.tree_clf_rand_bag_new_database.predict(self.X_database_test)
    self.y_pred_database = self.tree_clf_rand_bag_new_database.predict(self.X_database_test)
    self.y_pred_proba_database = self.tree_clf_rand_bag_new_database.predict_proba(self.X_database_test)
    with open(os.path.join(self.database, 'decisiontree_bag_randomsearch.txt'), 'a') as text_file:
    ...
    #self.y_pred_man_add = self.tree_clf_rand_bag_new_man_add.predict(self.X_man_add_test)
    self.y_pred_man_add = self.tree_clf_rand_bag_new_man_add.predict(self.X_man_add_test)
    self.y_pred_proba_man_add = self.tree_clf_rand_bag_new_man_add.predict_proba(self.X_man_add_test)
    with open(os.path.join(self.man_add, 'decisiontree_bag_randomsearch.txt'), 'a') as text_file:
    ...
    self.y_pred_transform = self.tree_clf_rand_bag_new_transform.predict(self.X_transform_test)
    self.y_pred_proba_transform = self.tree_clf_rand_bag_new_transform.predict_proba(self.X_transform_test)
    with open(os.path.join(self.transform, 'decisiontree_bag_randomsearch.txt'), 'a') as text_file:
    ...
    #self.y_pred_prot_screen_trans = self.tree_clf_rand_bag_new_prot_screen_trans.predict(self.X_prot_screen_trans_test)
    self.y_pred_prot_screen_trans = self.tree_clf_rand_bag_new_prot_screen_trans.predict(self.X_prot_screen_trans_test)
    self.y_pred_proba_prot_screen_trans = self.tree_clf_rand_bag_new_prot_screen_trans.predict_proba(self.X_prot_screen_trans_test)
    with open(os.path.join(self.prot_screen_trans, 'decisiontree_bag_randomsearch.txt'), 'a') as text_file:
    ...
    self.y_train_CV_pred_database = cross_val_predict(self.tree_clf_rand_bag_new_database, self.X_database_train, self.y_train, cv=10)
    self.y_train_CV_pred_proba_database = cross_val_predict(self.tree_clf_rand_bag_new_database, self.X_database_train, self.y_train, cv=10, method='predict_proba')
    with open(os.path.join(self.database, 'decisiontree_bag_randomsearch.txt'), 'a') as text_file:
      text_file.write('Saving predictions and probabilities for X_database_train with 10-fold CV in y_train_pred_database \n')
    self.y_train_CV_pred_man_add = cross_val_predict(self.tree_clf_rand_bag_new_man_add, self.X_man_add_train, self.y_train, cv=10)
    self.y_train_CV_pred_proba_man_add = cross_val_predict(self.tree_clf_rand_bag_new_man_add, self.X_man_add_train, self.y_train, cv=10, method='predict_proba')
    with open(os.path.join(self.man_add, 'decisiontree_bag_randomsearch.txt'), 'a') as text_file:
      text_file.write('Saving predictions and probabilities for X_man_add_train with 10-fold CV in y_train_pred_man_add \n')
    self.y_train_CV_pred_transform = cross_val_predict(self.tree_clf_rand_bag_new_transform, self.X_transform_train, self.y_train, cv=10)
    self.y_train_CV_pred_proba_transform = cross_val_predict(self.tree_clf_rand_bag_new_transform, self.X_transform_train, self.y_train, cv=10, method='predict_proba')
    with open(os.path.join(self.transform, 'decisiontree_bag_randomsearch.txt'), 'a') as text_file:
      text_file.write('Saving predictions and probabilities for X_transform_train with 10-fold CV in y_train_pred_transform \n')
    self.y_train_CV_pred_prot_screen_trans = cross_val_predict(self.tree_clf_rand_bag_new_prot_screen_trans, self.X_prot_screen_trans_train, self.y_train, cv=10)
    self.y_train_CV_pred_proba_prot_screen_trans = cross_val_predict(self.tree_clf_rand_bag_new_prot_screen_trans, self.X_prot_screen_trans_train, self.y_train, cv=10, method='predict_proba')
    with open(os.path.join(self.prot_screen_trans, 'decisiontree_bag_randomsearch.txt'), 'a') as text_file:
...
#      precision_recall(self.y_train, y_train_CV_pred_proba, 'train_CV_', directory)
    ...
    prediction_probas(self.tree_clf_rand_bag_new_database, self.X_database_train, self.y_train, self.X_database_test, self.y_test, self.y_pred_proba_database, self.y_train_CV_pred_proba_database, self.database, 'database')    
    prediction_probas(self.tree_clf_rand_bag_new_man_add, self.X_man_add_train, self.y_train, self.X_man_add_test, self.y_test, self.y_pred_proba_man_add, self.y_train_CV_pred_proba_man_add, self.man_add, 'man_add')    
    prediction_probas(self.tree_clf_rand_bag_new_transform, self.X_transform_train, self.y_train, self.X_transform_test, self.y_test, self.y_pred_proba_transform, self.y_train_CV_pred_proba_transform, self.transform, 'transform')    
    prediction_probas(self.tree_clf_rand_bag_new_prot_screen_trans, self.X_prot_screen_trans_train, self.y_train, self.X_prot_screen_trans_test, self.y_test, self.y_pred_proba_prot_screen_trans, self.y_train_CV_pred_proba_prot_screen_trans, self.prot_screen_trans, 'prot_screen_trans')  
