...
class RandomForestAdaGridSearch(object):
  ...
  '''
  def __init__(self, metrix, man_add_features, bbbb):
    self.metrix=metrix
    self.man_add_features=man_add_features
  ...
  def prepare_metrix_data(self):
    ...
    ******
    ...
    with open(os.path.join(self.man_add_features, 'randomforest_ada_gridsearch.txt'), 'a') as text_file:
      text_file.write('Preparing input data as metrix_man_add with following attributes %s \n' %(attr_man_add))
    
    metrix_man_add_normal = metrix_man_add[['anomalouscompl', 'anomalousslope', 'completeness', 'diffI']].copy()
    
    metrix_man_add_stratified = metrix_man_add[['cchalf', 'anomalousmulti']].copy()
    
    self.X_man_add = metrix_man_add_stratified
...
#stratified split of samples
  ...
  def rand_search(self):
    ...
    feature_importances_best_estimator(self.feature_importances_man_add_ls, 'man_add', self.man_add_features)
  ...
  def forest_best_params(self):
    ...
    self.tree_clf_grid_ada_new_man_add = AdaBoostClassifier(
    ...
    self.tree_clf_grid_ada_new_man_add.fit(self.X_man_add_train, self.y_train)
    ...
    feature_importances_pandas(self.tree_clf_grid_ada_new_man_add, self.X_man_add_train, 'man_add', self.man_add_features)
    ...
    write_pickle(self.tree_clf_grid_ada_new_man_add, self.man_add_features, 'man_add')
    ...
    visualise_tree(self.tree_clf_grid_ada_new_man_add, self.man_add_features, self.X_man_add_train.columns, 'man_add')
    ...
    basic_stats(self.tree_clf_grid_ada_new_man_add, self.X_man_add_train, self.man_add_features)
  ...
  def predict(self):
    ...
    #self.y_pred_man_add = self.tree_clf_grid_ada_new_man_add.predict(self.X_man_add_test)
    self.y_pred_man_add = self.tree_clf_grid_ada_new_man_add.predict(self.X_man_add_test)
    self.y_pred_proba_man_add = self.tree_clf_grid_ada_new_man_add.predict_proba(self.X_man_add_test)
    with open(os.path.join(self.man_add_features, 'randomforest_ada_gridsearch.txt'), 'a') as text_file:
    ...
    self.y_train_CV_pred_man_add = cross_val_predict(self.tree_clf_grid_ada_new_man_add, self.X_man_add_train, self.y_train, cv=10)
    self.y_train_CV_pred_proba_man_add = cross_val_predict(self.tree_clf_grid_ada_new_man_add, self.X_man_add_train, self.y_train, cv=10, method='predict_proba')
    with open(os.path.join(self.man_add_features, 'randomforest_ada_gridsearch.txt'), 'a') as text_file:
    ...
    prediction_stats(self.y_test, self.y_pred_man_add, self.man_add_features)
  ...
  def analysis(self):
    ...
    conf_mat(self.y_test, self.y_train, self.y_pred_man_add, self.y_train_CV_pred_man_add, self.man_add_features)
    ...
    prediction_probas(self.tree_clf_grid_ada_new_man_add, self.X_man_add_train, self.y_train, self.X_man_add_test, self.y_test, self.y_pred_proba_man_add, self.y_train_CV_pred_proba_man_add, self.man_add_features, 'man_add')    
...
def run():
  ...
  man_add_features, bbbb= make_output_folder(args.outdir)

  ###############################################################################

  random_forest_ada_grid_search = RandomForestAdaGridSearch(metrix, man_add_features, bbbb)
