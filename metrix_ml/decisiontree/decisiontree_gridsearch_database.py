...
class DecisionTreeGridSearchDatabase(object):
  ...
  def prepare_metrix_data(self):
    ...
    ******
    ...
    with open(os.path.join(self.database_only, 'decisiontree_gridsearch.txt'), 'a') as text_file:
...
#stratified split of samples
  ...
  def grid_search(self):
    ...
    self.feature_importances_database = feature_importances_database
  ...
  def tree_best_params(self):
    ...
    plot_features(self.tree_clf_grid_new_database, self.X_database_train.columns, self.database_only)
    ...
    write_pickle(self.tree_clf_grid_new_database, self.database_only, 'database')
    ...
    visualise_tree(self.tree_clf_grid_new_database, self.database_only, self.X_database_train.columns, 'database')

    print('*' *80)
    print('*    Getting basic stats using 10-fold cross-validation for new tree')
    print('*' *80)
    ...
    basic_stats(self.tree_clf_grid_new_database, self.X_database_train, self.database_only)
  ...
  def analysis(self):
    ...
    prediction_probas(self.tree_clf_grid_new_database, self.X_database_train, self.y_train, self.X_database_test, self.y_test, self.y_pred_proba_database, self.y_train_CV_pred_proba_database, self.database_only, 'database')    
...
def run():
  ...
  decision_tree_grid_search_database = DecisionTreeGridSearchDatabase(metrix, database_only, bbbb)
