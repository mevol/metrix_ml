...
class RandomForestAdaRandSearch(object):
  ...
  def prepare_metrix_data(self):
    ...
    ******
    ...
    with open(os.path.join(self.database_only, 'randomforest_ada_randomsearch.txt'), 'a') as text_file:
...
#stratified split of samples
  ...
  def forest_best_params(self):
    ...
    feature_importances_pandas(self.tree_clf_rand_ada_new_database, self.X_database_train, 'database', self.database_only)
    ...
    write_pickle(self.tree_clf_rand_ada_new_database, self.database_only, 'database')
    ...
    visualise_tree(self.tree_clf_rand_ada_new_database, self.database_only, self.X_database_train.columns, 'database')
    ...
    basic_stats(self.tree_clf_rand_ada_new_database, self.X_database_train, self.database_only)
  ...
  def analysis(self):
    ...
    prediction_probas(self.tree_clf_rand_ada_new_database, self.X_database_train, self.y_train, self.X_database_test, self.y_test, self.y_pred_proba_database, self.y_train_CV_pred_proba_database, self.database_only, 'database')    
...
def run():
  ...
  random_forest_ada_rand_search = RandomForestAdaRandSearch(metrix, database_only, bbbb)
