...
class DecisionTreeGridSearchTransform(object):
  ...
  def prepare_metrix_data(self):
    ...
    ******
    ...
    with open(os.path.join(self.transform_only, 'decisiontree_gridsearch.txt'), 'a') as text_file:
...
#     feature_importances_pandas(self.tree_clf_grid_new_prot_screen_trans, self.X_prot_screen_trans_train, 'prot_screen_trans', self.prot_screen_trans)
    ...
    plot_features(self.tree_clf_grid_new_transform, self.X_transform_train.columns, self.transform_only)
    ...
    write_pickle(self.tree_clf_grid_new_transform, self.transform_only, 'transform')
    ...
    visualise_tree(self.tree_clf_grid_new_transform, self.transform_only, self.X_transform_train.columns, 'transform')
    ...
    basic_stats(self.tree_clf_grid_new_transform, self.X_transform_train, self.transform_only)
  ...
  def analysis(self):
    ...
    prediction_probas(self.tree_clf_grid_new_transform, self.X_transform_train, self.y_train, self.X_transform_test, self.y_test, self.y_pred_proba_transform, self.y_train_CV_pred_proba_transform, self.transform_only, 'transform')    
...
def run():
  ...
  decision_tree_grid_search_transform = DecisionTreeGridSearchTransform(metrix, transform_only, bbbb)
