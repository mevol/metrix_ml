...
class DecisionTreeAdaRandSearch(object):
...
###############################################################################
  ...
  def rand_search(self):
    ...
    decision tree with AdaBoost which gives the best accuracy score'''
    print('*' *80)
    print('*    Running RandomizedSearch for best parameter combination for decision tree with AdaBoost')
    print('*' *80)
    ...
    with open(os.path.join(self.output_dir,
              'decisiontree_ada_randomsearch.txt'), 'a') as text_file:
      text_file.write('Created decision tree with AdaBoost: forest_clf_rand_ada \n')

    #set up randomized search
    param_rand = {"base_estimator__criterion": ["gini", "entropy"],#metric to judge reduction of impurity
                  'base_estimator__class_weight': ['balanced', None],
                  #'base_estimator__max_features': 2,#max number of features when splitting
                  ...
                  "base_estimator__max_depth": randint(1, 10),#max number of splits to do
    ...
    rand_search = RandomizedSearchCV(tree_clf_rand_ada,
                                     param_rand,
                                     random_state=5,
                                     cv=3,
                                     n_iter=500,
    ...
    ada_dict = rand_search_fitted.best_params_.copy()
...
###############################################################################
#
#  creating new tree with AdaBoost with best parameter combination
#
###############################################################################

  def forest_best_params(self):
    '''create a new decision tree with AdaBoost using the best parameter
       combination found above'''
    print('*' *80)
    print('*    Building new decision tree with AdaBoost on best parameter combination and save as pickle')
    print('*' *80)
    
    clf2 = DecisionTreeClassifier(**self.best_params_base_estimator,
                                  random_state= 0)
    self.tree_clf_rand_ada_new = AdaBoostClassifier(clf2,
...
#    feature_importances_best_estimator(feature_importances_transform_ls, 'newdata_minusEP', self.newdata_minusEP)
    ...
    def write_pickle(clf, directory):
      ...
      with open(os.path.join(directory,
                'decisiontree_ada_randomsearch.txt'), 'a') as text_file:
        text_file.write('Created new decision tree with AdaBoost "tree_clf_rand_ada_new" using best parameters \n')
        text_file.write('Creating pickle file for best forest as best_tree_clf_rand_ada_new.pkl \n')
    
    write_pickle(self.tree_clf_rand_ada_new,
                 self.output_dir)
    ...
    def visualise_tree(tree_forest, directory, columns):
      ...
      for tree in trees:
        with open(os.path.join(directory,
             'tree_clf_rand_ada_new_'+datestring+str(i_tree)+'.dot'), 'w') as f:
        ...
        dotfile = os.path.join(directory,
                         'tree_clf_rand_ada_new_'+datestring+str(i_tree)+'.dot')
        pngfile = os.path.join(directory,
                         'tree_clf_rand_ada_new_'+datestring+str(i_tree)+'.png')
      ...
      with open(os.path.join(directory,
                'decisiontree_ada_randomsearch.txt'), 'a') as text_file:
        text_file.write('Writing DOTfile and convert to PNG for "tree_clf_rand_ada_new" \n')
        text_file.write('DOT filename: tree_clf_rand_ada_new.dot \n')
    ...
    basic_stats(self.tree_clf_rand_ada_new,
                self.X_metrix_train,
                self.output_dir)
...
###############################################################################
  ...
  def predict(self):
    ...
    on the output'''
    print('*' *80)
    print('*    Predict using new decision tree with AdaBoost and test/train_CV set')
    print('*' *80)

    #try out how well the classifier works to predict from the test set
    self.y_pred = self.tree_clf_rand_ada_new.predict(self.X_metrix_test)
    self.y_pred_proba = self.tree_clf_rand_ada_new.predict_proba(self.X_metrix_test)
    with open(os.path.join(self.output_dir,
              'decisiontree_ada_randomsearch.txt'), 'a') as text_file:
    ...
    self.y_train_CV_pred = cross_val_predict(self.tree_clf_rand_ada_new,
                                             self.X_metrix_train,
                                             self.y_train, cv=3)
    self.y_train_CV_pred_proba = cross_val_predict(self.tree_clf_rand_ada_new,
                                                   self.X_metrix_train,
                                                   self.y_train,
                                                   cv=3,
...
###############################################################################
  ...
  def analysis(self):
    ...
    def prediction_probas(tree, X_train, y_train, X_test, y_test,
                                y_pred_proba, y_train_CV_pred_proba, directory): 
      datestring = datetime.strftime(datetime.now(), '%Y%m%d_%H%M')      
      with open(os.path.join(directory,
           'decisiontree_ada_randomsearch.txt'), 'a') as text_file:
...
#        text_file.write('AUC for CV train set class 1: %s \n' %AUC_train_class1)
    ...
    prediction_probas(self.tree_clf_rand_ada_new,
                      self.X_metrix_train,
                      self.y_train,
                      self.X_metrix_test,
                      self.y_test,
...
###############################################################################
  ...
  decision_tree_ada_rand_search = DecisionTreeAdaRandSearch(metrix, output_dir)
