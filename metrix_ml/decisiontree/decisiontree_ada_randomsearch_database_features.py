...
class RandomForestAdaRandSearch(object):
  ...
  def prepare_metrix_data(self):
    ...
    ******
    ...
    with open(os.path.join(self.database_features, 'randomforest_ada_randomsearch.txt'), 'a') as text_file:
      text_file.write('Preparing input data as metrix_database with following attributes %s \n' %(attr_database))
    
    metrix_database_normal = metrix_database[['anomalousslope', 'RpimdiffI', 'cchalf', 'diffF']].copy()
    
    metrix_database_stratified = metrix_database[['RpimdiffI', 'RpimI', 'IoverSigma', 'cchalf',]].copy()
...
#stratified split of samples
  ...
  def forest_best_params(self):
    ...
    feature_importances_pandas(self.tree_clf_rand_ada_new_database, self.X_database_train, 'database', self.database_features)
    ...
    write_pickle(self.tree_clf_rand_ada_new_database, self.database_features, 'database')
    ...
    visualise_tree(self.tree_clf_rand_ada_new_database, self.database_features, self.X_database_train.columns, 'database')
    ...
    basic_stats(self.tree_clf_rand_ada_new_database, self.X_database_train, self.database_features)
  ...
  def predict(self):
    ...
    #self.y_pred_database = self.tree_clf_rand_ada_new_database.predict(self.X_database_test)
    self.y_pred_database = self.tree_clf_rand_ada_new_database.predict(self.X_database_test)
    self.y_pred_proba_database = self.tree_clf_rand_ada_new_database.predict_proba(self.X_database_test)
    with open(os.path.join(self.database_features, 'randomforest_ada_randomsearch.txt'), 'a') as text_file:
    ...
    self.y_train_CV_pred_database = cross_val_predict(self.tree_clf_rand_ada_new_database, self.X_database_train, self.y_train, cv=10)
    self.y_train_CV_pred_proba_database = cross_val_predict(self.tree_clf_rand_ada_new_database, self.X_database_train, self.y_train, cv=10, method='predict_proba')
    with open(os.path.join(self.database_features, 'randomforest_ada_randomsearch.txt'), 'a') as text_file:
  ...
  def analysis(self):
    ...
    prediction_probas(self.tree_clf_rand_ada_new_database, self.X_database_train, self.y_train, self.X_database_test, self.y_test, self.y_pred_proba_database, self.y_train_CV_pred_proba_database, self.database_features, 'database')    
...
def run():
  ...
  random_forest_ada_rand_search = RandomForestAdaRandSearch(metrix, database_features, bbbb)
