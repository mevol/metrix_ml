...
class RandomForestAdaGridSearch(object):
  ...
  '''
  def __init__(self, metrix, man_add_only, bbbb):
    self.metrix=metrix
    self.man_add_only=man_add_only
  ...
  def prepare_metrix_data(self):
    ...
    ******
    ...
    with open(os.path.join(self.man_add_only, 'randomforest_ada_gridsearch.txt'), 'a') as text_file:
...
#    X_man_add_train, X_man_add_test, y_train, y_test = train_test_split(self.X_man_add, y, test_size=0.2, random_state=42, stratify=y)
  ...
  def rand_search(self):
    ...
    feature_importances_best_estimator(self.feature_importances_man_add_ls, 'man_add', self.man_add_only)
  ...
  def forest_best_params(self):
    ...
    feature_importances_pandas(self.tree_clf_grid_ada_new_man_add, self.X_man_add_train, 'man_add', self.man_add_only)
    ...
    write_pickle(self.tree_clf_grid_ada_new_man_add, self.man_add_only, 'man_add')
    ...
    visualise_tree(self.tree_clf_grid_ada_new_man_add, self.man_add_only, self.X_man_add_train.columns, 'man_add')
    ...
    basic_stats(self.tree_clf_grid_ada_new_man_add, self.X_man_add_train, self.man_add_only)
  ...
  def predict(self):
    ...
    prediction_stats(self.y_test, self.y_pred_man_add, self.man_add_only)
  ...
  def analysis(self):
    ...
    conf_mat(self.y_test, self.y_train, self.y_pred_man_add, self.y_train_CV_pred_man_add, self.man_add_only)
    ...
    prediction_probas(self.tree_clf_grid_ada_new_man_add, self.X_man_add_train, self.y_train, self.X_man_add_test, self.y_test, self.y_pred_proba_man_add, self.y_train_CV_pred_proba_man_add, self.man_add_only, 'man_add')    
...
def run():
  ...
  man_add_only, bbbb= make_output_folder(args.outdir)

  ###############################################################################

  random_forest_ada_grid_search = RandomForestAdaGridSearch(metrix, man_add_only, bbbb)
