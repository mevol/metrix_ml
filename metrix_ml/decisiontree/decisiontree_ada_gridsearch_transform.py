...
class RandomForestAdaGridSearch(object):
  ...
  '''
  def __init__(self, metrix, transform_only, bbbb):
    self.metrix=metrix
  ...
  def prepare_metrix_data(self):
    ...
    ******
    ...
    with open(os.path.join(self.transform_only, 'randomforest_ada_gridsearch.txt'), 'a') as text_file:
...
#    X_transform_train, X_transform_test, y_train, y_test = train_test_split(self.X_transform, y, test_size=0.2, random_state=42, stratify=y)
  ...
  def rand_search(self):
    ...
    param_grid = {"criterion": ["gini", "entropy"],
                 'max_features': [1, 2, 4, 8, 16, 21, 29, 39],
    ...
    feature_importances_best_estimator(self.feature_importances_transform_ls, 'transform', self.transform_only)
  ...
  def forest_best_params(self):
    ...
    feature_importances_pandas(self.tree_clf_grid_ada_new_transform, self.X_transform_train, 'transform', self.transform_only)
    ...
    write_pickle(self.tree_clf_grid_ada_new_transform, self.transform_only, 'transform')
    ...
    visualise_tree(self.tree_clf_grid_ada_new_transform, self.transform_only, self.X_transform_train.columns, 'transform')
    ...
    basic_stats(self.tree_clf_grid_ada_new_transform, self.X_transform_train, self.transform_only)
  ...
  def predict(self):
    ...
    prediction_stats(self.y_test, self.y_pred_transform, self.transform_only)
  ...
  def analysis(self):
    ...
    conf_mat(self.y_test, self.y_train, self.y_pred_transform, self.y_train_CV_pred_transform, self.transform_only)
    ...
    prediction_probas(self.tree_clf_grid_ada_new_transform, self.X_transform_train, self.y_train, self.X_transform_test, self.y_test, self.y_pred_proba_transform, self.y_train_CV_pred_proba_transform, self.transform_only, 'transform')    
...
def run():
  ...
  transform_only, bbbb= make_output_folder(args.outdir)

  ###############################################################################

  random_forest_ada_grid_search = RandomForestAdaGridSearch(metrix, transform_only, bbbb)
