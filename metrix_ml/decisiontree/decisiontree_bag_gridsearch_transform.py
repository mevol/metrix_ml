...
class DecisionTreeBagGridSearch(object):
  ...
  def prepare_metrix_data(self):
    ...
    ******
    ...
    with open(os.path.join(self.transform_only, 'decisiontree_bag_gridsearch.txt'), 'a') as text_file:
...
#    X_transform_train, X_transform_test, y_train, y_test = train_test_split(self.X_transform, y, test_size=0.2, random_state=42, stratify=y)
  ...
  def tree_best_params(self):
    ...
    feature_importances_pandas(self.tree_clf_grid_bag_new_transform, self.X_transform_train, 'transform', self.transform_only)     
    ...
    write_pickle(self.tree_clf_grid_bag_new_transform, self.transform_only, 'transform')
    ...
    visualise_tree(self.tree_clf_grid_bag_new_transform, self.transform_only, self.X_transform_train.columns, 'transform')
    ...
    basic_stats(self.tree_clf_grid_bag_new_transform, self.X_transform_train, self.transform_only)
  ...
  def predict(self):
    ...
    with open(os.path.join(self.transform_only, 'decisiontree_bag_randomsearch.txt'), 'a') as text_file:
  ...
  def analysis(self):
    ...
    prediction_probas(self.tree_clf_grid_bag_new_transform, self.X_transform_train, self.y_train, self.X_transform_test, self.y_test, self.y_pred_proba_transform, self.y_train_CV_pred_proba_transform, self.transform_only, 'transform')    
...
def run():
  ...
  decision_tree_bag_grid_search = DecisionTreeBagGridSearch(metrix, transform_only, bbbb)
