...
class DecisionTreeBagGridSearch(object):
  ...
  def prepare_metrix_data(self):
    ...
    ******
    ...
    with open(os.path.join(self.database_features, 'decisiontree_bag_gridsearch.txt'), 'a') as text_file:
      text_file.write('Preparing input data as metrix_database with following attributes %s \n' %(attr_database))
    
    metrix_database_normal = metrix_database[['RpimdiffI', 'lowreslimit', 'totalunique', 'anomalousmulti', 'IoverSigma']].copy()
    metrix_database_stratified = metrix_database[['RpimdiffI', 'totalobservations', 'anomalousmulti']].copy()
...
#    X_database_train, X_database_test, y_train, y_test = train_test_split(self.X_database, y, test_size=0.2, random_state=42, stratify=y)
  ...
  def tree_best_params(self):
    ...
    feature_importances_pandas(self.tree_clf_grid_bag_new_database, self.X_database_train, 'database', self.database_features)
    ...
    write_pickle(self.tree_clf_grid_bag_new_database, self.database_features, 'database')
    ...
    visualise_tree(self.tree_clf_grid_bag_new_database, self.database_features, self.X_database_train.columns, 'database')
    ...
    basic_stats(self.tree_clf_grid_bag_new_database, self.X_database_train, self.database_features)
  ...
  def predict(self):
    ...
    with open(os.path.join(self.database_features, 'decisiontree_bag_randomsearch.txt'), 'a') as text_file:
  ...
  def analysis(self):
    ...
    prediction_probas(self.tree_clf_grid_bag_new_database, self.X_database_train, self.y_train, self.X_database_test, self.y_test, self.y_pred_proba_database, self.y_train_CV_pred_proba_database, self.database_features, 'database')    
...
def run():
  ...
  decision_tree_bag_grid_search = DecisionTreeBagGridSearch(metrix, database_features, bbbb)
