...
class RandomForestAdaRandSearch(object):
  ...
  def prepare_metrix_data(self):
    ...
    ******
    ...
    with open(os.path.join(self.man_add_features, 'randomforest_ada_randomsearch.txt'), 'a') as text_file:
      text_file.write('Preparing input data as metrix_man_add with following attributes %s \n' %(attr_man_add))
    
    metrix_man_add_normal = metrix_man_add[['diffI', 'diffF', 'anomalousslope', 'Matth_coeff']].copy()
    
    metrix_man_add_stratified = metrix_man_add[['lowreslimit', 'Vcell', 'sites_ASU', 'cchalf']].copy()
...
#stratified split of samples
  ...
  def forest_best_params(self):
    ...
    feature_importances_pandas(self.tree_clf_rand_ada_new_man_add, self.X_man_add_train, 'man_add', self.man_add_features)
    ...
    write_pickle(self.tree_clf_rand_ada_new_man_add, self.man_add_features, 'man_add')
    ...
    visualise_tree(self.tree_clf_rand_ada_new_man_add, self.man_add_features, self.X_man_add_train.columns, 'man_add')
    ...
    basic_stats(self.tree_clf_rand_ada_new_man_add, self.X_man_add_train, self.man_add_features)
  ...
  def predict(self):
    ...
    #self.y_pred_man_add = self.tree_clf_rand_ada_new_man_add.predict(self.X_man_add_test)
    self.y_pred_man_add = self.tree_clf_rand_ada_new_man_add.predict(self.X_man_add_test)
    self.y_pred_proba_man_add = self.tree_clf_rand_ada_new_man_add.predict_proba(self.X_man_add_test)
    with open(os.path.join(self.man_add_features, 'randomforest_ada_randomsearch.txt'), 'a') as text_file:
    ...
    self.y_train_CV_pred_man_add = cross_val_predict(self.tree_clf_rand_ada_new_man_add, self.X_man_add_train, self.y_train, cv=10)
    self.y_train_CV_pred_proba_man_add = cross_val_predict(self.tree_clf_rand_ada_new_man_add, self.X_man_add_train, self.y_train, cv=10, method='predict_proba')
    with open(os.path.join(self.man_add_features, 'randomforest_ada_randomsearch.txt'), 'a') as text_file:
  ...
  def analysis(self):
    ...
    prediction_probas(self.tree_clf_rand_ada_new_man_add, self.X_man_add_train, self.y_train, self.X_man_add_test, self.y_test, self.y_pred_proba_man_add, self.y_train_CV_pred_proba_man_add, self.man_add_features, 'man_add')    
...
def run():
  ...
  random_forest_ada_rand_search = RandomForestAdaRandSearch(metrix, man_add_features, bbbb)
