...
class DecisionTreeRandomSearchTransform(object):
  ...
  def prepare_metrix_data(self):
    ...
    ******
    ...
    with open(os.path.join(self.transform_features, 'decisiontree_randomsearch.txt'), 'a') as text_file:
    ...
    metrix_transform_features_normal = metrix_transform[['RpimdiffI', 'MW_chain/No_atom_chain', 'Matth_coeff', 'totalobservations']].copy()
    
    metrix_transform_features_stratified = metrix_transform[['RpimdiffI', 'No_atom_chain', 'Matth_coeff', 'totalobservations', 'Vcell/Vm<Ma>']].copy()
...
#    X_transform_train, X_transform_test, y_train, y_test = train_test_split(self.X_transform, y, test_size=0.2, random_state=42, stratify=y)
  ...
  def tree_best_params(self):
    ...
    plot_features(self.tree_clf_rand_new_transform, self.X_transform_train.columns, self.transform_features)
    ...
    write_pickle(self.tree_clf_rand_new_transform, self.transform_features, 'transform')
    ...
    visualise_tree(self.tree_clf_rand_new_transform, self.transform_features, self.X_transform_train.columns, 'transform')
    ...
    basic_stats(self.tree_clf_rand_new_transform, self.X_transform_train, self.transform_features)
  ...
  def analysis(self):
    ...
    prediction_probas(self.tree_clf_rand_new_transform, self.X_transform_train, self.y_train, self.X_transform_test, self.y_test, self.y_pred_proba_transform, self.y_train_CV_pred_proba_transform, self.transform_features, 'transform')    
...
def run():
  ...
  decision_tree_random_search_transform = DecisionTreeRandomSearchTransform(metrix, transform_features, bbbb)
