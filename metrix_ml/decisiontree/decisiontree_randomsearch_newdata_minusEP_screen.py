...
class DecisionTreeRandomSearchTransform(object):
  ...
  def prepare_metrix_data(self):
    ...
    ******
    ...
    self.X_newdata_screen = metrix_newdata_transform[['highreslimit', 'wavelength', 'wavelength**3', 'wavelength**3/Vcell',
                      'sg_number', 'cell_a', 'cell_b', 'cell_c', 'cell_alpha',
                      'cell_beta', 'cell_gamma','Vcell', 'solvent_content',
                      'Vcell/Vm<Ma>', 'Matth_coeff', 'MW_ASU/sites_ASU/solvent_content',
                      'MW_chain', 'No_atom_chain', 'No_mol_ASU', 'MW_ASU', 'sites_ASU',
    ...
    self.X_newdata_screen = self.X_newdata_screen.fillna(0)

    with open(os.path.join(self.newdata_minusEP, 'decisiontree_randomsearch.txt'), 'a') as text_file:
      text_file.write('Created the following dataframes: metrix_transform \n')
      text_file.write(str(self.X_newdata_screen.columns)+'\n')    
...
#stratified split of samples
    X_newdata_transform_train, X_newdata_transform_test, y_train, y_test = train_test_split(self.X_newdata_screen, y, test_size=0.2, random_state=42, stratify=y)
    
    assert self.X_newdata_screen.columns.all() == X_newdata_transform_train.columns.all()
  ...
  def rand_search(self):
    ...
    param_rand = {"criterion": ["gini", "entropy"],#metric to judge reduction of impurity
                  'max_features': randint(2, 24),#max number of features when splitting
  ...
  def tree_best_params(self):
    ...
    def plot_features(clf, attr, directory):
      ...
      skplt.estimators.plot_feature_importances(clf, feature_names=attr, x_tick_rotation=60, max_num_features=24, figsize=(25, 25))
      plt.tight_layout()
