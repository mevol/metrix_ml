...
###############################################################################
#
#  randomised search for best parameter combination
#
###############################################################################
  ...
  def rand_search(self):
    ...
    tree_clf_ada = AdaBoostClassifier(base_estimator = clf1,
                                      algorithm = "SAMME.R",
                                      random_state = 100)

    logging.info(f"Created blank decision tree with AdaBoost")

    #set up randomized search
    param_dict = {"base_estimator__criterion": ["gini", "entropy"],
    ...
    logging.info(
    f"Running randomised search for hyperparameters for blank decision tree with AdaBoost \n"
    f"using the following parameters {param_dict}")
    ...
    best_score = round(rand_search_fitted.best_score_, 2)
...
###############################################################################
#
#  editing parameter dictionary to be used in next step
#
###############################################################################
...
###############################################################################
#
#  creating new classifier with best parameter combination found above
#
###############################################################################
  ...
  def forest_best_params(self):
    ...
    logging.info(f"Created classifier based on best paramter combination found\n"
                 f"and fitting training data.")
...
###############################################################################
    ...
    def basic_stats(forest, data_train, labels_train, directory):
      ...
      accuracy_mean_cv = round(float(cross_val_score(forest, data_train,
