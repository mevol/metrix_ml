...
class DecisionTreeRandomSearchDatabase(object):
  ...
  def prepare_metrix_data(self):
    ...
    ******
    ...
    with open(os.path.join(self.database_only, 'decisiontree_randomsearch.txt'), 'a') as text_file:
...
#stratified split of samples
  ...
  def rand_search(self):
    ...
    param_rand = {"criterion": ["gini", "entropy"],#metric to judge reduction of impurity
                  'max_features': randint(2, 21),#max number of features when splitting
  ...
  def tree_best_params(self):
    ...
    plot_features(self.tree_clf_rand_new_database, self.X_database_train.columns, self.database_only)
    ...
    write_pickle(self.tree_clf_rand_new_database, self.database_only, 'database')
    ...
    visualise_tree(self.tree_clf_rand_new_database, self.database_only, self.X_database_train.columns, 'database')
    ...
    basic_stats(self.tree_clf_rand_new_database, self.X_database_train, self.database_only)
  ...
  def analysis(self):
    ...
    prediction_probas(self.tree_clf_rand_new_database, self.X_database_train, self.y_train, self.X_database_test, self.y_test, self.y_pred_proba_database, self.y_train_CV_pred_proba_database, self.database_only, 'database')    
...
def run():
  ...
  decision_tree_random_search_database = DecisionTreeRandomSearchDatabase(metrix, database_only, bbbb)
