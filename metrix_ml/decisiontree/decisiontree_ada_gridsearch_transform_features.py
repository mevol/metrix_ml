...
class RandomForestAdaGridSearch(object):
  ...
  '''
  def __init__(self, metrix, transform_features, bbbb):
    self.metrix=metrix
  ...
  def prepare_metrix_data(self):
    ...
    ******
    ...
    with open(os.path.join(self.transform_features, 'randomforest_ada_gridsearch.txt'), 'a') as text_file:
    ...
    metrix_transform_normal = metrix_transform[['RpimdiffI', 'anomalouscompl', 'cchalf', 'anomalousCC', 'solvent_content', 'MW_chain/No_atom_chain', 'anomalousslope', 'diffI']].copy()
    
    metrix_transform_stratified = metrix_transform[['cchalf', 'anomalousmulti', 'RpimI', 'MW_ASU/sites_ASU']].copy()
       
    self.X_transform = metrix_transform_stratified
...
#stratified split of samples
  ...
  def rand_search(self):
    ...
    feature_importances_best_estimator(self.feature_importances_transform_ls, 'transform', self.transform_features)
  ...
  def forest_best_params(self):
    ...
    self.tree_clf_grid_ada_new_transform = AdaBoostClassifier(
    ...
    self.tree_clf_grid_ada_new_transform.fit(self.X_transform_train, self.y_train)
    ...
    feature_importances_pandas(self.tree_clf_grid_ada_new_transform, self.X_transform_train, 'transform', self.transform_features)
    ...
    write_pickle(self.tree_clf_grid_ada_new_transform, self.transform_features, 'transform')
    ...
    visualise_tree(self.tree_clf_grid_ada_new_transform, self.transform_features, self.X_transform_train.columns, 'transform')
    ...
    basic_stats(self.tree_clf_grid_ada_new_transform, self.X_transform_train, self.transform_features)
  ...
  def predict(self):
    ...
    #self.y_pred_transform = self.tree_clf_grid_ada_new_transform.predict(self.X_transform_test)
    self.y_pred_transform = self.tree_clf_grid_ada_new_transform.predict(self.X_transform_test)
    self.y_pred_proba_transform = self.tree_clf_grid_ada_new_transform.predict_proba(self.X_transform_test)
    with open(os.path.join(self.transform_features, 'randomforest_ada_gridsearch.txt'), 'a') as text_file:
    ...
    self.y_train_CV_pred_transform = cross_val_predict(self.tree_clf_grid_ada_new_transform, self.X_transform_train, self.y_train, cv=10)
    self.y_train_CV_pred_proba_transform = cross_val_predict(self.tree_clf_grid_ada_new_transform, self.X_transform_train, self.y_train, cv=10, method='predict_proba')
    with open(os.path.join(self.transform_features, 'randomforest_ada_gridsearch.txt'), 'a') as text_file:
    ...
    prediction_stats(self.y_test, self.y_pred_transform, self.transform_features)
  ...
  def analysis(self):
    ...
    conf_mat(self.y_test, self.y_train, self.y_pred_transform, self.y_train_CV_pred_transform, self.transform_features)
    ...
    prediction_probas(self.tree_clf_grid_ada_new_transform, self.X_transform_train, self.y_train, self.X_transform_test, self.y_test, self.y_pred_proba_transform, self.y_train_CV_pred_proba_transform, self.transform_features, 'transform')    
...
def run():
  ...
  transform_features, bbbb= make_output_folder(args.outdir)

  ###############################################################################

  random_forest_ada_grid_search = RandomForestAdaGridSearch(metrix, transform_features, bbbb)
