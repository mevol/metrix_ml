...
class RandomForestAdaGridSearch(object):
  ...
  '''
  def __init__(self, metrix, database_features, bbbb):
    self.metrix=metrix
  ...
  def prepare_metrix_data(self):
    ...
    ******
    ...
    with open(os.path.join(self.database_features, 'randomforest_ada_gridsearch.txt'), 'a') as text_file:
      text_file.write('Preparing input data as metrix_database with following attributes %s \n' %(attr_database))
    
    metrix_database_normal = metrix_database[['RpimdiffI', 'cchalf', 'diffI', 'RpimI', 'IoverSigma', 'lowreslimit']].copy()
    
    metrix_database_stratified = metrix_database[['totalunique', 'totalobservations', 'RpimdiffI', 'RmeasdiffI']].copy()
    
    self.X_database = metrix_database_stratified
...
#stratified split of samples
  ...
  def rand_search(self):
    ...
    feature_importances_best_estimator(self.feature_importances_database_ls, 'database', self.database_features)
  ...
  def forest_best_params(self):
    ...
    self.tree_clf_grid_ada_new_database = AdaBoostClassifier(
    ...
    self.tree_clf_grid_ada_new_database.fit(self.X_database_train, self.y_train)
    ...
    feature_importances_pandas(self.tree_clf_grid_ada_new_database, self.X_database_train, 'database', self.database_features)
    ...
    write_pickle(self.tree_clf_grid_ada_new_database, self.database_features, 'database')
    ...
    visualise_tree(self.tree_clf_grid_ada_new_database, self.database_features, self.X_database_train.columns, 'database')
    ...
    basic_stats(self.tree_clf_grid_ada_new_database, self.X_database_train, self.database_features)
  ...
  def predict(self):
    ...
    #self.y_pred_database = self.tree_clf_grid_ada_new_database.predict(self.X_database_test)
    self.y_pred_database = self.tree_clf_grid_ada_new_database.predict(self.X_database_test)
    self.y_pred_proba_database = self.tree_clf_grid_ada_new_database.predict_proba(self.X_database_test)
    with open(os.path.join(self.database_features, 'randomforest_ada_gridsearch.txt'), 'a') as text_file:
      text_file.write('Saving predictions and probabilities for X_database_test in y_pred_database and probabilities y_pred_proba_database\n')

    #alternative way to not have to use the test set
    self.y_train_CV_pred_database = cross_val_predict(self.tree_clf_grid_ada_new_database, self.X_database_train, self.y_train, cv=10)
    self.y_train_CV_pred_proba_database = cross_val_predict(self.tree_clf_grid_ada_new_database, self.X_database_train, self.y_train, cv=10, method='predict_proba')
    with open(os.path.join(self.database_features, 'randomforest_ada_gridsearch.txt'), 'a') as text_file:
    ...
    prediction_stats(self.y_test, self.y_pred_database, self.database_features)
  ...
  def analysis(self):
    ...
    conf_mat(self.y_test, self.y_train, self.y_pred_database, self.y_train_CV_pred_database, self.database_features)
    ...
    prediction_probas(self.tree_clf_grid_ada_new_database, self.X_database_train, self.y_train, self.X_database_test, self.y_test, self.y_pred_proba_database, self.y_train_CV_pred_proba_database, self.database_features, 'database')    
...
def run():
  ...
  database_features, bbbb= make_output_folder(args.outdir)

  ###############################################################################

  random_forest_ada_grid_search = RandomForestAdaGridSearch(metrix, database_features, bbbb)
