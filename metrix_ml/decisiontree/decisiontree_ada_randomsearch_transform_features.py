...
class RandomForestAdaRandSearch(object):
  ...
  def prepare_metrix_data(self):
    ...
    ******
    ...
    with open(os.path.join(self.transform_features, 'randomforest_ada_randomsearch.txt'), 'a') as text_file:
    ...
    metrix_transform_normal = metrix_transform[['RpimdiffI', 'RpimI', 'solvent_content', 'wilsonbfactor']].copy()
    
    metrix_transform_stratified = metrix_transform[['RpimdiffI', 'MW_chain/No_atom_chain', 'anomalousslope', 'diffF', 'cchalf']].copy()
...
#stratified split of samples
  ...
  def forest_best_params(self):
    ...
    feature_importances_pandas(self.tree_clf_rand_ada_new_transform, self.X_transform_train, 'transform', self.transform_features)
    ...
    write_pickle(self.tree_clf_rand_ada_new_transform, self.transform_features, 'transform')
    ...
    visualise_tree(self.tree_clf_rand_ada_new_transform, self.transform_features, self.X_transform_train.columns, 'transform')
    ...
    basic_stats(self.tree_clf_rand_ada_new_transform, self.X_transform_train, self.transform_features)
  ...
  def predict(self):
    ...
    #self.y_pred_transform = self.tree_clf_rand_ada_new_transform.predict(self.X_transform_test)
    self.y_pred_transform = self.tree_clf_rand_ada_new_transform.predict(self.X_transform_test)
    self.y_pred_proba_transform = self.tree_clf_rand_ada_new_transform.predict_proba(self.X_transform_test)
    with open(os.path.join(self.transform_features, 'randomforest_ada_randomsearch.txt'), 'a') as text_file:
    ...
    self.y_train_CV_pred_transform = cross_val_predict(self.tree_clf_rand_ada_new_transform, self.X_transform_train, self.y_train, cv=10)
    self.y_train_CV_pred_proba_transform = cross_val_predict(self.tree_clf_rand_ada_new_transform, self.X_transform_train, self.y_train, cv=10, method='predict_proba')
    with open(os.path.join(self.transform_features, 'randomforest_ada_randomsearch.txt'), 'a') as text_file:
  ...
  def analysis(self):
    ...
    prediction_probas(self.tree_clf_rand_ada_new_transform, self.X_transform_train, self.y_train, self.X_transform_test, self.y_test, self.y_pred_proba_transform, self.y_train_CV_pred_proba_transform, self.transform_features, 'transform')    
...
def run():
  ...
  random_forest_ada_rand_search = RandomForestAdaRandSearch(metrix, transform_features, bbbb)
