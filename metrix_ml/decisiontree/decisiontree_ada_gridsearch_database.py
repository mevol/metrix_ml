...
class RandomForestAdaGridSearch(object):
  ...
  '''
  def __init__(self, metrix, database_only, bbbb):
    self.metrix=metrix
  ...
  def prepare_metrix_data(self):
    ...
    ******
    ...
    with open(os.path.join(self.database_only, 'randomforest_ada_gridsearch.txt'), 'a') as text_file:
...
#    X_database_train, X_database_test, y_train, y_test = train_test_split(self.X_database, y, test_size=0.2, random_state=42, stratify=y)
  ...
  def rand_search(self):
    ...
    feature_importances_best_estimator(self.feature_importances_database_ls, 'database', self.database_only)
  ...
  def forest_best_params(self):
    ...
    feature_importances_pandas(self.tree_clf_grid_ada_new_database, self.X_database_train, 'database', self.database_only)
    ...
    write_pickle(self.tree_clf_grid_ada_new_database, self.database_only, 'database')
    ...
    visualise_tree(self.tree_clf_grid_ada_new_database, self.database_only, self.X_database_train.columns, 'database')
    ...
    basic_stats(self.tree_clf_grid_ada_new_database, self.X_database_train, self.database_only)
  ...
  def predict(self):
    ...
    prediction_stats(self.y_test, self.y_pred_database, self.database_only)
  ...
  def analysis(self):
    ...
    conf_mat(self.y_test, self.y_train, self.y_pred_database, self.y_train_CV_pred_database, self.database_only)
    ...
    prediction_probas(self.tree_clf_grid_ada_new_database, self.X_database_train, self.y_train, self.X_database_test, self.y_test, self.y_pred_proba_database, self.y_train_CV_pred_proba_database, self.database_only, 'database')    
...
def run():
  ...
  database_only, bbbb= make_output_folder(args.outdir)

  ###############################################################################

  random_forest_ada_grid_search = RandomForestAdaGridSearch(metrix, database_only, bbbb)
