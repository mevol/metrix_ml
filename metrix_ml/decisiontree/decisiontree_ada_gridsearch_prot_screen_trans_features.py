...
def make_output_folder(outdir):
  names = ['prot_screen_trans_features', 'bbbb']
  result = []
  for name in names:
...
class RandomForestAdaGridSearch(object):
  ...
  '''
  def __init__(self, metrix, prot_screen_trans_features, bbbb):
    self.metrix=metrix
    self.prot_screen_trans_features=prot_screen_trans_features
  ...
  def prepare_metrix_data(self):
    ...
    ******
    ...
    with open(os.path.join(self.prot_screen_trans_features, 'randomforest_ada_gridsearch.txt'), 'a') as text_file:
    ...
    metrix_prot_screen_trans_normal = metrix_prot_screen_trans[['Matth_coeff', 'solvent_content', 'MW_ASU/sites_ASU']].copy()

    metrix_prot_screen_trans_stratified = metrix_prot_screen_trans[['MW_chain/No_atom_chain', 'MW_chain', 'No_atom_chain']].copy()

    self.X_prot_screen_trans = metrix_prot_screen_trans_stratified
...
#stratified split of samples
  ...
  def rand_search(self):
    ...
    tree_clf_grid_ada = DecisionTreeClassifier(random_state=42)
    ...
    grid_search = RandomizedSearchCV(tree_clf_grid_ada, param_grid, random_state=5,
    ...
    def feature_importances_best_estimator(feature_list, name, directory):
      ...
      plt.xlabel('Features')
      plt.tight_layout()
      plt.savefig(os.path.join(directory, 'feature_importances_best_bar_plot_grid_ada_'+name+datestring+'.png'))     
      plt.close()
    
    feature_importances_best_estimator(self.feature_importances_prot_screen_trans_ls, 'prot_screen_trans', self.prot_screen_trans_features)
  ...
  def forest_best_params(self):
    ...
    self.tree_clf_grid_ada_new_prot_screen_trans = AdaBoostClassifier(
                                DecisionTreeClassifier(**self.best_params_prot_screen_trans, random_state=42),
                                algorithm ="SAMME.R", random_state=5, learning_rate=0.01, n_estimators=500)
    self.tree_clf_grid_ada_new_prot_screen_trans.fit(self.X_prot_screen_trans_train, self.y_train)
    ...
    def feature_importances_pandas(clf, X_train, name, directory):   
      ...
      plt.xlabel('Features')
      plt.tight_layout()
      plt.savefig(os.path.join(directory, 'feature_importances_overall_bar_plot_grid_ada_'+name+datestring+'.png'))
      plt.close()
      
    feature_importances_pandas(self.tree_clf_grid_ada_new_prot_screen_trans, self.X_prot_screen_trans_train, 'prot_screen_trans', self.prot_screen_trans_features)
    ...
    write_pickle(self.tree_clf_grid_ada_new_prot_screen_trans, self.prot_screen_trans_features, 'prot_screen_trans')
    ...
    visualise_tree(self.tree_clf_grid_ada_new_prot_screen_trans, self.prot_screen_trans_features, self.X_prot_screen_trans_train.columns, 'prot_screen_trans')
    ...
    basic_stats(self.tree_clf_grid_ada_new_prot_screen_trans, self.X_prot_screen_trans_train, self.prot_screen_trans_features)
  ...
  def predict(self):
    ...
    #self.y_pred_prot_screen_trans = self.tree_clf_grid_ada_new_prot_screen_trans.predict(self.X_prot_screen_trans_test)
    self.y_pred_prot_screen_trans = self.tree_clf_grid_ada_new_prot_screen_trans.predict(self.X_prot_screen_trans_test)
    self.y_pred_proba_prot_screen_trans = self.tree_clf_grid_ada_new_prot_screen_trans.predict_proba(self.X_prot_screen_trans_test)
    with open(os.path.join(self.prot_screen_trans_features, 'randomforest_ada_gridsearch.txt'), 'a') as text_file:
      text_file.write('Saving predictions and probabilities for X_prot_screen_trans_test in y_pred_prot_screen_trans and probabilities y_pred_proba_prot_screen_trans\n')

    #alternative way to not have to use the test set
    self.y_train_CV_pred_prot_screen_trans = cross_val_predict(self.tree_clf_grid_ada_new_prot_screen_trans, self.X_prot_screen_trans_train, self.y_train, cv=10)
    self.y_train_CV_pred_proba_prot_screen_trans = cross_val_predict(self.tree_clf_grid_ada_new_prot_screen_trans, self.X_prot_screen_trans_train, self.y_train, cv=10, method='predict_proba')
    with open(os.path.join(self.prot_screen_trans_features, 'randomforest_ada_gridsearch.txt'), 'a') as text_file:
    ...
    prediction_stats(self.y_test, self.y_pred_prot_screen_trans, self.prot_screen_trans_features)
  ...
  def analysis(self):
    ...
    conf_mat(self.y_test, self.y_train, self.y_pred_prot_screen_trans, self.y_train_CV_pred_prot_screen_trans, self.prot_screen_trans_features)
    ...
    def prediction_probas(tree, X_train, y_train, X_test, y_test, y_pred_proba, y_train_CV_pred_proba, directory, kind): 
      ...
      def plot_precision_recall_vs_threshold(precisions, recalls, thresholds_forest, name, classes, directory):
        ...
        plt.ylim([0,1])
        plt.savefig(os.path.join(directory, 'Precision_Recall_forest_grid_ada_'+name+datestring+classes+'.png'))
        plt.close()
     ...
     #plot Precision Recall Threshold curve for test set        
      ...
      def plot_roc_curve(fpr, tpr, name, classes, directory):
        ...
        plt.grid(True)
        plt.savefig(os.path.join(directory, 'ROC_curve_forest_grid_ada_'+name+datestring+classes+'.png'))
        plt.close()
    ...
    prediction_probas(self.tree_clf_grid_ada_new_prot_screen_trans, self.X_prot_screen_trans_train, self.y_train, self.X_prot_screen_trans_test, self.y_test, self.y_pred_proba_prot_screen_trans, self.y_train_CV_pred_proba_prot_screen_trans, self.prot_screen_trans_features, 'prot_screen_trans')    
...
def run():
  ...
  prot_screen_trans_features, bbbb= make_output_folder(args.outdir)

  ###############################################################################

  random_forest_ada_grid_search = RandomForestAdaGridSearch(metrix, prot_screen_trans_features, bbbb)
