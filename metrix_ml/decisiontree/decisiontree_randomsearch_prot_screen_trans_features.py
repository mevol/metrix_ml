...
class DecisionTreeRandomSearchProtScreenTrans(object):
  ...
  def prepare_metrix_data(self):
    ...
    ******
    ...
    with open(os.path.join(self.prot_screen_trans_features, 'decisiontree_randomsearch.txt'), 'a') as text_file:
    ...
    metrix_prot_screen_trans_features_normal = metrix_prot_screen_trans[['MW_ASU/sites_ASU', 'MW_ASU/sites_ASU/solvent_content', 'MW_chain/No_atom_chain', 'highreslimit']].copy()
    
    metrix_prot_screen_trans_features_stratified = metrix_prot_screen_trans[['MW_chain/No_atom_chain', 'MW_ASU/sites_ASU', 'highreslimit', 'No_atom_chain', 'sites_ASU']].copy()
    
    self.X_prot_screen_trans = metrix_prot_screen_trans_features_normal
...
#    X_prot_screen_trans_train, X_prot_screen_trans_test, y_train, y_test = train_test_split(self.X_prot_screen_trans, y, test_size=0.2, random_state=42, stratify=y)
  ...
  def rand_search(self):
    ...
    param_rand = {"criterion": ["gini", "entropy"],#metric to judge reduction of impurity
                  'max_features': randint(2, 4),#max number of features when splitting
  ...
  def tree_best_params(self):
    ...
    plot_features(self.tree_clf_rand_new_prot_screen_trans, self.X_prot_screen_trans_train.columns, self.prot_screen_trans_features)
    ...
    write_pickle(self.tree_clf_rand_new_prot_screen_trans, self.prot_screen_trans_features, 'prot_screen_trans')
    ...
    visualise_tree(self.tree_clf_rand_new_prot_screen_trans, self.prot_screen_trans_features, self.X_prot_screen_trans_train.columns, 'prot_screen_trans')
    ...
    basic_stats(self.tree_clf_rand_new_prot_screen_trans, self.X_prot_screen_trans_train, self.prot_screen_trans_features)
  ...
  def analysis(self):
    ...
    prediction_probas(self.tree_clf_rand_new_prot_screen_trans, self.X_prot_screen_trans_train, self.y_train, self.X_prot_screen_trans_test, self.y_test, self.y_pred_proba_prot_screen_trans, self.y_train_CV_pred_proba_prot_screen_trans, self.prot_screen_trans_features, 'prot_screen_trans')  
...
def run():
  ...
  decision_tree_random_search_prot_screen_trans = DecisionTreeRandomSearchProtScreenTrans(metrix, prot_screen_trans_features, bbbb)
