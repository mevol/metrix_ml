...
class ExtremeRandomForestGridSearch(object):
  ...
  def prepare_metrix_data(self):
    ...
    ******
    ...
    with open(os.path.join(self.prot_screen_trans_only, 'extreme_randomforest_gridsearch.txt'), 'a') as text_file:
      text_file.write('Preparing input data as metrix_prot_screen_trans with following attributes %s \n' %(attr_prot_screen_trans))
...
#    X_prot_screen_trans_train, X_prot_screen_trans_test, y_train, y_test = train_test_split(self.X_prot_screen_trans, y, test_size=0.2, random_state=42, stratify=y)
  ...
  def forest_best_params(self):
    ...
    self.extra_clf_grid_new_prot_screen_trans = ExtraTreesClassifier(**self.best_params_prot_screen_trans, random_state=42)
    self.extra_clf_grid_new_prot_screen_trans.fit(self.X_prot_screen_trans_train, self.y_train)
    ...
    feature_importances_pandas(self.extra_clf_grid_new_prot_screen_trans, self.X_prot_screen_trans_train, 'prot_screen_trans', self.prot_screen_trans_only)
    ...
    write_pickle(self.extra_clf_grid_new_prot_screen_trans, self.prot_screen_trans_only, 'prot_screen_trans')
    ...
    visualise_tree(self.extra_clf_grid_new_prot_screen_trans, self.prot_screen_trans_only, self.X_prot_screen_trans_train.columns, 'prot_screen_trans')
    ...
    basic_stats(self.extra_clf_grid_new_prot_screen_trans, self.X_prot_screen_trans_train, self.prot_screen_trans_only)
  ...
  def predict(self):
    ...
    #self.y_pred_prot_screen_trans = self.extra_clf_grid_new_prot_screen_trans.predict(self.X_prot_screen_trans_test)
    self.y_pred_prot_screen_trans = self.extra_clf_grid_new_prot_screen_trans.predict(self.X_prot_screen_trans_test)
    self.y_pred_proba_prot_screen_trans = self.extra_clf_grid_new_prot_screen_trans.predict_proba(self.X_prot_screen_trans_test)
    with open(os.path.join(self.prot_screen_trans_only, 'extreme_randomforest_gridsearch.txt'), 'a') as text_file:
      text_file.write('Saving predictions and probabilities for X_prot_screen_trans_test in y_pred_prot_screen_trans and probabilities in y_pred_proba_prot_screen_trans \n')

    #alternative way to not have to use the test set
    self.y_train_CV_pred_prot_screen_trans = cross_val_predict(self.extra_clf_grid_new_prot_screen_trans, self.X_prot_screen_trans_train, self.y_train, cv=10)
    self.y_train_CV_pred_proba_prot_screen_trans = cross_val_predict(self.extra_clf_grid_new_prot_screen_trans, self.X_prot_screen_trans_train, self.y_train, cv=10, method='predict_proba')
    with open(os.path.join(self.prot_screen_trans_only, 'extreme_randomforest_gridsearch.txt'), 'a') as text_file:
  ...
  def analysis(self):
    ...
    prediction_probas(self.extra_clf_grid_new_prot_screen_trans, self.X_prot_screen_trans_train, self.y_train, self.X_prot_screen_trans_test, self.y_test, self.y_pred_proba_prot_screen_trans, self.y_train_CV_pred_proba_prot_screen_trans, self.prot_screen_trans_only, 'prot_screen_trans')    
...
def run():
  ...
  extreme_random_forest_grid_search = ExtremeRandomForestGridSearch(metrix, prot_screen_trans_only, bbbb)
