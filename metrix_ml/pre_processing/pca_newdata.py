...
class FeatureDecomposition(object):
  ...
  def prepare_metrix_data(self):
    ...
    ******
    ...
    with open(os.path.join(self.newdata, 'pca.txt'), 'a') as text_file:
...
#stratified split of samples
    X_newdata_initial_train, X_newdata_initial_test, y_train, y_test = train_test_split(self.X_newdata_transform, y, test_size=0.2, random_state=42, stratify=y)
    
    assert self.X_newdata_transform.columns.all() == X_newdata_initial_train.columns.all()

    self.X_newdata_initial_train = X_newdata_initial_train
    self.X_newdata_initial_test = X_newdata_initial_test
...
###############################################################################
    ...
    #standardise data
    X_newdata_initial_train_std = StandardScaler().fit_transform(self.X_newdata_initial_train)
    self.X_transform_newdata_initial_std = X_newdata_initial_train_std
...
#        print(i[0])
      ...
      PCs = ['PC-1','PC-2', 'PC-3', 'PC-4', 'PC-5', 'PC-6', 'PC-7', 'PC-8',
             ...
             'PC-44', 'PC-45', 'PC-46', 'PC-47', 'PC-48', 'PC-49', 'PC-50',
             'PC-51', 'PC-52', 'PC-53', 'PC-54', 'PC-55', 'PC-56', 'PC-57',
             'PC-58', 'PC-59']   
      ...
      plt.savefig(os.path.join(self.newdata, 'PCA_cumulative_explained_variance_transform_'+datestring+'.png'))     
      plt.close()

    pca_manual(self.X_transform_newdata_initial_std)
...
###############################################################################
    ...
    max_num_pc = run_pca_reduced(self.X_transform_newdata_initial_std)
...
#      print(pca.singular_values_)
     ...
     # Dimension indexing
      ...
      plt.tight_layout()
      plt.savefig(os.path.join(self.newdata, 'Feature_weights_per_PC_transform_'+datestring+'.png'))
      plt.close()
    ...
    pca_results = pca_results(self.X_transform_newdata_initial_std, self.X_newdata_initial_train.columns, max_num_pc)
    #print(pca_results)
...
###############################################################################
    ...
    def run_pca_pair_one(X_train_std, columns):
      ...
      def biplot(X_train_std, reduced_X_train_std, pca, columns):
        ...
        plt.savefig(os.path.join(self.newdata, 'PC_biplot_PC1_PC2_transform_'+datestring+'.png')) 
        return ax

      biplot(X_train_std, reduced_X_train_std, pca, columns)

    run_pca_pair_one(self.X_transform_newdata_initial_std, self.X_newdata_initial_train.columns)
...
###############################################################################    
    ...
    def feature_weights(X_train_std, columns, num_pc):
      ...
      p1_i = plt.bar(ind_i, df_i.iloc[0], width)
      p2_i = plt.bar(ind_i, df_i.iloc[1], width)
      p3_i = plt.bar(ind_i, df_i.iloc[2], width)
      p4_i = plt.bar(ind_i, df_i.iloc[3], width)
      p5_i = plt.bar(ind_i, df_i.iloc[4], width)
      p6_i = plt.bar(ind_i, df_i.iloc[5], width)
      p7_i = plt.bar(ind_i, df_i.iloc[6], width)
      p8_i = plt.bar(ind_i, df_i.iloc[7], width)
      p9_i = plt.bar(ind_i, df_i.iloc[8], width)
      p10_i = plt.bar(ind_i, df_i.iloc[9], width)
      p11_i = plt.bar(ind_i, df_i.iloc[10], width)
      p12_i = plt.bar(ind_i, df_i.iloc[11], width)
      p13_i = plt.bar(ind_i, df_i.iloc[12], width)
      p14_i = plt.bar(ind_i, df_i.iloc[13], width)
      p15_i = plt.bar(ind_i, df_i.iloc[14], width)
      p16_i = plt.bar(ind_i, df_i.iloc[15], width)
      p17_i = plt.bar(ind_i, df_i.iloc[16], width)
      p18_i = plt.bar(ind_i, df_i.iloc[17], width)
      p19_i = plt.bar(ind_i, df_i.iloc[18], width)
      p20_i = plt.bar(ind_i, df_i.iloc[19], width)
      p21_i = plt.bar(ind_i, df_i.iloc[20], width)
      p22_i = plt.bar(ind_i, df_i.iloc[21], width)
      p23_i = plt.bar(ind_i, df_i.iloc[22], width)
      p24_i = plt.bar(ind_i, df_i.iloc[23], width)
      p25_i = plt.bar(ind_i, df_i.iloc[24], width)
      p26_i = plt.bar(ind_i, df_i.iloc[25], width)
      p27_i = plt.bar(ind_i, df_i.iloc[26], width)
      p28_i = plt.bar(ind_i, df_i.iloc[27], width)
      p29_i = plt.bar(ind_i, df_i.iloc[28], width)
      p30_i = plt.bar(ind_i, df_i.iloc[29], width)
      p31_i = plt.bar(ind_i, df_i.iloc[30], width)
      p32_i = plt.bar(ind_i, df_i.iloc[31], width)
      p33_i = plt.bar(ind_i, df_i.iloc[32], width)
      p34_i = plt.bar(ind_i, df_i.iloc[33], width)
      p35_i = plt.bar(ind_i, df_i.iloc[34], width)
      p36_i = plt.bar(ind_i, df_i.iloc[35], width)
      p37_i = plt.bar(ind_i, df_i.iloc[36], width)
      p38_i = plt.bar(ind_i, df_i.iloc[37], width)
      p39_i = plt.bar(ind_i, df_i.iloc[38], width)
      p40_i = plt.bar(ind_i, df_i.iloc[39], width)
      p41_i = plt.bar(ind_i, df_i.iloc[40], width)
      p42_i = plt.bar(ind_i, df_i.iloc[41], width)
      p43_i = plt.bar(ind_i, df_i.iloc[42], width)
      p44_i = plt.bar(ind_i, df_i.iloc[43], width)
      p45_i = plt.bar(ind_i, df_i.iloc[44], width)      
      p46_i = plt.bar(ind_i, df_i.iloc[45], width)
      p47_i = plt.bar(ind_i, df_i.iloc[46], width)
      p48_i = plt.bar(ind_i, df_i.iloc[47], width)
      p49_i = plt.bar(ind_i, df_i.iloc[48], width)
      p50_i = plt.bar(ind_i, df_i.iloc[49], width)
      p51_i = plt.bar(ind_i, df_i.iloc[50], width)
      p52_i = plt.bar(ind_i, df_i.iloc[51], width)
      p53_i = plt.bar(ind_i, df_i.iloc[52], width)
      p54_i = plt.bar(ind_i, df_i.iloc[53], width)
      p55_i = plt.bar(ind_i, df_i.iloc[54], width)
      p56_i = plt.bar(ind_i, df_i.iloc[55], width)
      p57_i = plt.bar(ind_i, df_i.iloc[56], width)
      p58_i = plt.bar(ind_i, df_i.iloc[57], width)
      p59_i = plt.bar(ind_i, df_i.iloc[58], width)
      
      plt.title('Feature dominance in each PC')
      plt.xlabel('Number of PCs')
      plt.ylabel('Contribution of individual features in PC')
      plt.xticks(ind_i, lab_i, rotation=90)
      plt.legend(labels=df_i.index, loc='right')
      plt.tight_layout()
      plt.savefig(os.path.join(self.newdata, 'PC_feature_contribution_transform_'+datestring+'.png'))     
      plt.close()
      
    feature_weights(self.X_transform_newdata_initial_std, self.X_newdata_initial_train.columns, max_num_pc)     
...
def run():
  ...
  feature_decomposition = FeatureDecomposition(metrix, newdata, bbbb)
