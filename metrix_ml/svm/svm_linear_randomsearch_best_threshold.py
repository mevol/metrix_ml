...
def make_output_folder(outdir):
  ...
  result = []
  for name in names:
    name = os.path.join(outdir, 'svm_randomsearch', name)
...
#stratified split of samples
    ...
    #print('scaled columns length: %s' %str(self.X_newdata_transform_train.shape))
  ...
  def svm_best_params(self):
    ...
    self.svc_clf_grid_new_transform = SVC(C=1.3227176189929446,
    ...
    #print(cv)
    #cv.fit(['diffI', 'anomalousCC', 'MW_ASU/sites_ASU', 'lowreslimit', 'anomalousslope', 'diffF', 'MW_ASU/sites_ASU/solvent_content', 'MW_chain/No_atom_chain',
    #'Matth_coeff', 'cell_a', 'sg_number', 'cchalf', 'anomalouscompl', 'solvent_content', 'volume_wilsonB_highres'])
    cv.fit(['diffI', 'anomalousCC', 'lowreslimit', 'anomalousslope', 'diffF'])
    #print(cv)                  
    ...
    def plot_coefficients(coef, feature_names, top_features=8):
  ...
  def analysis(self):
    ...
    def conf_mat(y_test, y_train, y_pred, y_train_pred, directory):
      ...
      def draw_conf_mat(matrix, directory, name):
        ...
        plt.ylabel('True')
        plt.savefig(os.path.join(directory, 'confusion_matrix_svm_grid_'+name+datestring+'.png'), dpi=600)
        plt.close()
     ...
     #plot Precision Recall Threshold curve for test set        
      ...
      def evaluate_threshold(tpr, fpr, thresholds, threshold, name, directory):
        ...
        with open(os.path.join(directory, 'svm_randomsearch.txt'), 'a') as text_file:
          ...
          #text_file.write('True-positive rate %d for %s at threshold %.2f: %s \n' %(tpr, name, threshold, sensitivity))
          #text_file.write('False-positive rate for %d%s at threshold %.2f: %s \n' %(fpr, name, threshold, specificity))
...
def run():
  ...
  svm_grid_search = SVMGridSearch(metrix, newdata_minusEP, bbbb)
