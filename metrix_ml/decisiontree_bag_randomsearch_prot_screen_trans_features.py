...
class DecisionTreeBagRandomSearch(object):
  ...
  def prepare_metrix_data(self):
    ...
    ******
    ...
    with open(os.path.join(self.prot_screen_trans_features, 'decisiontree_bag_randomsearch.txt'), 'a') as text_file:
    ...
    metrix_prot_screen_trans_stratified = metrix_prot_screen_trans[['MW_chain/No_atom_chain', 'MW_ASU/sites_ASU', 'No_atom_chain', 'highreslimit', 'sites_ASU']].copy()
...
#    X_prot_screen_trans_train, X_prot_screen_trans_test, y_train, y_test = train_test_split(self.X_prot_screen_trans, y, test_size=0.2, random_state=42, stratify=y)
  ...
  def tree_best_params(self):
    ...
    feature_importances_pandas(self.tree_clf_rand_bag_new_prot_screen_trans, self.X_prot_screen_trans_train, 'prot_screen_trans', self.prot_screen_trans_features)
    ...
    write_pickle(self.tree_clf_rand_bag_new_prot_screen_trans, self.prot_screen_trans_features, 'prot_screen_trans')
    ...
    visualise_tree(self.tree_clf_rand_bag_new_prot_screen_trans, self.prot_screen_trans_features, self.X_prot_screen_trans_train.columns, 'prot_screen_trans')
    ...
    basic_stats(self.tree_clf_rand_bag_new_prot_screen_trans, self.X_prot_screen_trans_train, self.prot_screen_trans_features)
  ...
  def analysis(self):
    ...
    prediction_probas(self.tree_clf_rand_bag_new_prot_screen_trans, self.X_prot_screen_trans_train, self.y_train, self.X_prot_screen_trans_test, self.y_test, self.y_pred_proba_prot_screen_trans, self.y_train_CV_pred_proba_prot_screen_trans, self.prot_screen_trans_features, 'prot_screen_trans')  
...
def run():
  ...
  decision_tree_bag_rand_search = DecisionTreeBagRandomSearch(metrix, prot_screen_trans_features, bbbb)
