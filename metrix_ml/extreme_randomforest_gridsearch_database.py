...
class ExtremeRandomForestGridSearch(object):
  ...
  def prepare_metrix_data(self):
    ...
    ******
    ...
    with open(os.path.join(self.database_only, 'extreme_randomforest_gridsearch.txt'), 'a') as text_file:
...
#    X_database_train, X_database_test, y_train, y_test = train_test_split(self.X_database, y, test_size=0.2, random_state=42, stratify=y)
  ...
  def forest_best_params(self):
    ...
    self.extra_clf_grid_new_database = ExtraTreesClassifier(**self.best_params_database, random_state=42)
    self.extra_clf_grid_new_database.fit(self.X_database_train, self.y_train)
    ...
    feature_importances_pandas(self.extra_clf_grid_new_database, self.X_database_train, 'database', self.database_only)
    ...
    write_pickle(self.extra_clf_grid_new_database, self.database_only, 'database')
    ...
    visualise_tree(self.extra_clf_grid_new_database, self.database_only, self.X_database_train.columns, 'database')
    ...
    basic_stats(self.extra_clf_grid_new_database, self.X_database_train, self.database_only)
  ...
  def predict(self):
    ...
    #self.y_pred_database = self.extra_clf_grid_new_database.predict(self.X_database_test)
    self.y_pred_database = self.extra_clf_grid_new_database.predict(self.X_database_test)
    self.y_pred_proba_database = self.extra_clf_grid_new_database.predict_proba(self.X_database_test)
    with open(os.path.join(self.database_only, 'extreme_randomforest_gridsearch.txt'), 'a') as text_file:
    ...
    self.y_train_CV_pred_database = cross_val_predict(self.extra_clf_grid_new_database, self.X_database_train, self.y_train, cv=10)
    self.y_train_CV_pred_proba_database = cross_val_predict(self.extra_clf_grid_new_database, self.X_database_train, self.y_train, cv=10, method='predict_proba')
    with open(os.path.join(self.database_only, 'extreme_randomforest_gridsearch.txt'), 'a') as text_file:
  ...
  def analysis(self):
    ...
    prediction_probas(self.extra_clf_grid_new_database, self.X_database_train, self.y_train, self.X_database_test, self.y_test, self.y_pred_proba_database, self.y_train_CV_pred_proba_database, self.database_only, 'database')    
...
def run():
  ...
  extreme_random_forest_grid_search = ExtremeRandomForestGridSearch(metrix, database_only, bbbb)
