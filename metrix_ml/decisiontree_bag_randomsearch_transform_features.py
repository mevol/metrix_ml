...
class DecisionTreeBagRandomSearch(object):
  ...
  def prepare_metrix_data(self):
    ...
    ******
    ...
    metrix_transform_normal = metrix_transform[['RpimdiffI', 'totalobservations', 'Matth_coeff', 'MW_chain/No_atom_chain', 'anomalousmulti']].copy()
    
    metrix_transform_stratified = metrix_transform[['RpimdiffI', 'No_atom_chain', 'Matth_coeff', 'totalobservations', 'Vcell/Vm<Ma>']].copy()
...
#    X_transform_train, X_transform_test, y_train, y_test = train_test_split(self.X_transform, y, test_size=0.2, random_state=42, stratify=y)
  ...
  def tree_best_params(self):
    ...
    feature_importances_pandas(self.tree_clf_rand_bag_new_transform, self.X_transform_train, 'transform', self.transform_features)
    ...
    write_pickle(self.tree_clf_rand_bag_new_transform, self.transform_features, 'transform')
    ...
    visualise_tree(self.tree_clf_rand_bag_new_transform, self.transform_features, self.X_transform_train.columns, 'transform')
    ...
    basic_stats(self.tree_clf_rand_bag_new_transform, self.X_transform_train, self.transform_features)
  ...
  def analysis(self):
    ...
    prediction_probas(self.tree_clf_rand_bag_new_transform, self.X_transform_train, self.y_train, self.X_transform_test, self.y_test, self.y_pred_proba_transform, self.y_train_CV_pred_proba_transform, self.transform_features, 'transform')    
...
def run():
  ...
  decision_tree_bag_rand_search = DecisionTreeBagRandomSearch(metrix, transform_features, bbbb)
