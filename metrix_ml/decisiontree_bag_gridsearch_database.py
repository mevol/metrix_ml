...
class DecisionTreeBagGridSearch(object):
  ...
  def prepare_metrix_data(self):
    ...
    ******
    ...
    with open(os.path.join(self.database_only, 'decisiontree_bag_gridsearch.txt'), 'a') as text_file:
...
#stratified split of samples
  ...
  def tree_best_params(self):
    ...
    feature_importances_pandas(self.tree_clf_grid_bag_new_database, self.X_database_train, 'database', self.database_only)
    ...
    write_pickle(self.tree_clf_grid_bag_new_database, self.database_only, 'database')
    ...
    visualise_tree(self.tree_clf_grid_bag_new_database, self.database_only, self.X_database_train.columns, 'database')
    ...
    basic_stats(self.tree_clf_grid_bag_new_database, self.X_database_train, self.database_only)
  ...
  def predict(self):
    ...
    with open(os.path.join(self.database_only, 'decisiontree_bag_randomsearch.txt'), 'a') as text_file:
  ...
  def analysis(self):
    ...
    prediction_probas(self.tree_clf_grid_bag_new_database, self.X_database_train, self.y_train, self.X_database_test, self.y_test, self.y_pred_proba_database, self.y_train_CV_pred_proba_database, self.database_only, 'database')    
...
def run():
  ...
  decision_tree_bag_grid_search = DecisionTreeBagGridSearch(metrix, database_only, bbbb)
