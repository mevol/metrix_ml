...
class RandomForestAdaGridSearch(object):
  ...
  def forest_best_params(self):
    ...
    i_tree = 0
    for tree in trees:
      with open(os.path.join(self.outdir,'forest_clf_grid_ada_new_tree' + str(i_tree) + '.dot'), 'w') as f:
        export_graphviz(tree, out_file=f, feature_names=self.X_transform_train.columns,
                   rounded=True, filled=True)
        f.close()
      dotfile = os.path.join(self.outdir, 'forest_clf_grid_ada_new_tree' + str(i_tree) + '.dot')
      pngfile = os.path.join(self.outdir, 'forest_clf_grid_ada_new_tree' + str(i_tree) + '.png')
    ...
    accuracy_each_cv = cross_val_score(self.forest_clf_grid_ada_new, self.X_transform_train, self.y_train,
                    cv=10, scoring='accuracy')
    accuracy_mean_cv = cross_val_score(self.forest_clf_grid_ada_new, self.X_transform_train, self.y_train,
                    cv=10, scoring='accuracy').mean()
    # calculate cross_val_scoring with different scoring functions for CV train set
    train_roc_auc = cross_val_score(self.forest_clf_grid_ada_new, self.X_transform_train, self.y_train, cv=10,
                    scoring='roc_auc').mean()
    train_accuracy = cross_val_score(self.forest_clf_grid_ada_new, self.X_transform_train, self.y_train, cv=10,
                    scoring='accuracy').mean()
    train_recall = cross_val_score(self.forest_clf_grid_ada_new, self.X_transform_train, self.y_train, cv=10,
                    scoring='recall').mean()
  ...
  def predict(self):
    ...
    self.y_pred_class = self.forest_clf_grid_ada_new.predict(self.X_transform_test)
    with open(os.path.join(self.outdir, 'randomforest_ada_gridsearch.txt'), 'a') as text_file:
      text_file.write('Saving predictions for X_transform_test in y_pred_class \n')

    #alternative way to not have to use the test set
    self.y_train_pred = cross_val_predict(self.forest_clf_grid_ada_new, self.X_transform_train, self.y_train,
                      cv=10)
    with open(os.path.join(self.outdir, 'randomforest_ada_gridsearch.txt'), 'a') as text_file:
  ...
  def analysis(self):
    ...
    self.y_pred_proba_train_CV = cross_val_predict(self.forest_clf_grid_ada_new, self.X_transform_train, self.y_train, cv=10, method='predict_proba')
...
#    self.y_scores=self.forest_clf_grid_ada_grid_new.predict_proba(self.X_transform_train)#train set
    ...
    def plot_hist_pred_proba(y_pred_proba, name):
      ...
      plt.savefig(os.path.join(self.outdir, 'hist_pred_proba_forest_grid_ada_'+name+datestring+'.png'))
      plt.close()
    ...
    def plot_precision_recall_vs_threshold(precisions, recalls, thresholds_forest, name):
        ...
        plt.ylim([0,1])
        plt.savefig(os.path.join(self.outdir, 'Precision_Recall_forest_grid_ada_'+name+datestring+'.png'))
        plt.close()
    ...
    def plot_roc_curve(fpr, tpr, name, label=None):
        ...
        plt.grid(True)
        plt.savefig(os.path.join(self.outdir, 'ROC_curve_forest_grid_ada_'+name+datestring+'.png'))
        plt.close()
    ...
    def scoring(X, y, name, cv ):
      ...
      roc_auc = cross_val_score(self.forest_clf_grid_ada_new, X, y, cv=cv,
                      scoring='roc_auc').mean()
      accuracy = cross_val_score(self.forest_clf_grid_ada_new, X, y, cv=cv,
                      scoring='accuracy').mean()
      recall = cross_val_score(self.forest_clf_grid_ada_new, X, y, cv=cv,
                      scoring='recall').mean()
      precision = cross_val_score(self.forest_clf_grid_ada_new, X, y, cv=cv,
                      scoring='precision').mean()
      f1 = cross_val_score(self.forest_clf_grid_ada_new, X, y, cv=cv,
                      scoring='f1').mean()
      with open(os.path.join(self.outdir, 'randomforest_ada_gridsearch.txt'), 'a') as text_file:
...
def run():
  ...
  random_forest_ada_grid_search = RandomForestAdaGridSearch(metrix, args.outdir)
