...
def make_output_folder(outdir):
  ...
  result = []
  for name in names:
    name = os.path.join(outdir, 'randomforest_ada_gridsearch', name)
...
class RandomForestAdaGridSearch(object):
  ...
  def prepare_metrix_data(self):
    ...
    ******
    ...
    with open(os.path.join(self.database, 'randomforest_ada_gridsearch.txt'), 'a') as text_file:
    ...
    with open(os.path.join(self.man_add, 'randomforest_ada_gridsearch.txt'), 'a') as text_file:
    ...
    with open(os.path.join(self.transform, 'randomforest_ada_gridsearch.txt'), 'a') as text_file:
      text_file.write('Preparing input data as metrix_transform with following attributes %s \n' %(attr_transform))

    with open(os.path.join(self.prot_screen_trans, 'randomforest_ada_gridsearch.txt'), 'a') as text_file:
  ...
  def grid_search(self):
    ...
    grid_search = GridSearchCV(forest_clf_grid_ada, param_grid, cv=10, scoring='accuracy')
  ...
  def forest_best_params(self):
    ...
    self.forest_clf_grid_ada_new_database = AdaBoostClassifier(
                                RandomForestClassifier(**self.best_params_database, random_state=42),
                                algorithm ="SAMME.R", learning_rate=0.5)
    self.forest_clf_grid_ada_new_database.fit(self.X_database_train, self.y_train)

    self.forest_clf_grid_ada_new_man_add = AdaBoostClassifier(
                                RandomForestClassifier(**self.best_params_man_add, random_state=42),
                                algorithm ="SAMME.R", learning_rate=0.5)
    self.forest_clf_grid_ada_new_man_add.fit(self.X_man_add_train, self.y_train)

    self.forest_clf_grid_ada_new_transform = AdaBoostClassifier(
                                RandomForestClassifier(**self.best_params_transform, random_state=42),
                                algorithm ="SAMME.R", learning_rate=0.5)
    self.forest_clf_grid_ada_new_transform.fit(self.X_transform_train, self.y_train)

    self.forest_clf_grid_ada_new_prot_screen_trans = AdaBoostClassifier(
                                RandomForestClassifier(**self.best_params_prot_screen_trans, random_state=42),
                                algorithm ="SAMME.R", learning_rate=0.5)
    self.forest_clf_grid_ada_new_prot_screen_trans.fit(self.X_prot_screen_trans_train, self.y_train)
    ...
    def feature_importances(clf, X_train, directory):
     ...
     # Plot the feature importances of the forest
      ...
      plt.savefig(os.path.join(directory, 'feature_importances_bar_plot_grid_ada_'+datestring+'.png'))
      plt.close()

    feature_importances(self.forest_clf_grid_ada_new_database, self.X_database_train, self.database)
    feature_importances(self.forest_clf_grid_ada_new_man_add, self.X_man_add_train, self.man_add)
    feature_importances(self.forest_clf_grid_ada_new_transform, self.X_transform_train, self.transform)
    feature_importances(self.forest_clf_grid_ada_new_prot_screen_trans, self.X_prot_screen_trans_train, self.prot_screen_trans)

    def write_pickle(forest, directory, name):
      datestring = datetime.strftime(datetime.now(), '%Y%m%d_%H%M')
      joblib.dump(forest, os.path.join(directory,'best_forest_grid_ada_'+name+datestring+'.pkl'))
      with open(os.path.join(directory, 'randomforest_ada_gridsearch.txt'), 'a') as text_file:
        text_file.write('Created new random forest "forest_clf_grid_ada_new_%s" using best parameters \n' %name)
        text_file.write('Creating pickle file for best forest as best_forest_grid_ada_%s.pkl \n' %name)
    
    write_pickle(self.forest_clf_grid_ada_new_database, self.database, 'database')
    write_pickle(self.forest_clf_grid_ada_new_man_add, self.man_add, 'man_add')
    write_pickle(self.forest_clf_grid_ada_new_transform, self.transform, 'transform')
    write_pickle(self.forest_clf_grid_ada_new_prot_screen_trans, self.prot_screen_trans, 'prot_screen_trans')
    ...
    def visualise_tree(tree_forest, directory, columns, name):
      ...
      i_tree = 0
      for tree in trees:
        with open(os.path.join(directory,'tree_clf_grid_ada_new_'+name+datestring+str(i_tree)+'.dot'), 'w') as f:
          export_graphviz(tree, out_file=f, feature_names=columns, rounded=True, filled=True)
          f.close()
        dotfile = os.path.join(directory, 'tree_clf_grid_ada_new_'+name+datestring+str(i_tree)+'.dot')
        pngfile = os.path.join(directory, 'tree_clf_grid_ada_new_'+name+datestring+str(i_tree)+'.png')
        command = ["dot", "-Tpng", dotfile, "-o", pngfile]
        subprocess.check_call(command)
        i_tree = i_tree + 1

      with open(os.path.join(directory, 'randomforest_ada_gridsearch.txt'), 'a') as text_file:
        text_file.write('Writing DOTfile and convert to PNG for "tree_clf_grid_ada_new_%s" \n' %name)
        text_file.write('DOT filename: tree_clf_grid_ada_new_%s.dot \n' %name)
        text_file.write('PNG filename: tree_clf_grid_ada_new_%s.png \n' %name)

    
    visualise_tree(self.forest_clf_grid_ada_new_database, self.database, self.X_database_train.columns, 'database')
    visualise_tree(self.forest_clf_grid_ada_new_man_add, self.man_add, self.X_man_add_train.columns, 'man_add')
    visualise_tree(self.forest_clf_grid_ada_new_transform, self.transform, self.X_transform_train.columns, 'transform')
    visualise_tree(self.forest_clf_grid_ada_new_prot_screen_trans, self.prot_screen_trans, self.X_prot_screen_trans_train.columns, 'prot_screen_trans')
  ...
  def predict(self):
    ...
    self.y_pred_class_database = self.forest_clf_grid_ada_new_database.predict(self.X_database_test)
    with open(os.path.join(self.database, 'randomforest_ada_gridsearch.txt'), 'a') as text_file:
      text_file.write('Saving predictions for X_database_test in y_pred_class_database \n')
    self.y_pred_class_man_add = self.forest_clf_grid_ada_new_man_add.predict(self.X_man_add_test)
    with open(os.path.join(self.man_add, 'randomforest_ada_gridsearch.txt'), 'a') as text_file:
      text_file.write('Saving predictions for X_man_add_test in y_pred_class_man_add \n')
    self.y_pred_class_transform = self.forest_clf_grid_ada_new_transform.predict(self.X_transform_test)
    with open(os.path.join(self.transform, 'randomforest_ada_gridsearch.txt'), 'a') as text_file:
      text_file.write('Saving predictions for X_transform_test in y_pred_class_transform \n')
    self.y_pred_class_prot_screen_trans = self.forest_clf_grid_ada_new_prot_screen_trans.predict(self.X_prot_screen_trans_test)
    with open(os.path.join(self.prot_screen_trans, 'randomforest_ada_gridsearch.txt'), 'a') as text_file:
    ...
    self.y_train_pred_database = cross_val_predict(self.forest_clf_grid_ada_new_database, self.X_database_train, self.y_train, cv=10)
    with open(os.path.join(self.database, 'randomforest_ada_gridsearch.txt'), 'a') as text_file:
      text_file.write('Saving predictions for X_database_train with 10-fold CV in y_train_pred_database \n')
    self.y_train_pred_man_add = cross_val_predict(self.forest_clf_grid_ada_new_man_add, self.X_man_add_train, self.y_train, cv=10)
    with open(os.path.join(self.man_add, 'randomforest_ada_gridsearch.txt'), 'a') as text_file:
      text_file.write('Saving predictions for X_man_add_train with 10-fold CV in y_train_pred_man_add \n')
    self.y_train_pred_transform = cross_val_predict(self.forest_clf_grid_ada_new_transform, self.X_transform_train, self.y_train, cv=10)
    with open(os.path.join(self.transform, 'randomforest_ada_gridsearch.txt'), 'a') as text_file:
      text_file.write('Saving predictions for X_transform_train with 10-fold CV in y_train_pred_transform \n')
    self.y_train_pred_prot_screen_trans = cross_val_predict(self.forest_clf_grid_ada_new_prot_screen_trans, self.X_prot_screen_trans_train, self.y_train, cv=10)
    with open(os.path.join(self.prot_screen_trans, 'randomforest_ada_gridsearch.txt'), 'a') as text_file:
  ...
  def analysis(self):
    ...
    def conf_mat(y_test, y_train, y_pred_class, y_train_pred, directory):
      ...
      def draw_conf_mat(matrix, directory, name):
        ...
        plt.ylabel('True')
        plt.savefig(os.path.join(directory, 'confusion_matrix_forest_grid_ada_'+name+datestring+'.png'))
        plt.close()
    ...
    def prediction_probas(forest, X_train, y_train, X_test, y_test, directory, kind): 
      ...
      def plot_hist_pred_proba(y_pred_proba, name, directory):
        ...
        plt.savefig(os.path.join(directory, 'hist_pred_proba_forest_grid_ada_'+name+datestring+'.png'))
        plt.close()
      ...
      def plot_precision_recall_vs_threshold(precisions, recalls, thresholds_forest, name, directory):
        ...
        plt.ylim([0,1])
        plt.savefig(os.path.join(directory, 'Precision_Recall_forest_grid_ada_'+name+datestring+'.png'))
        plt.close()
      ...
      def plot_roc_curve(fpr, tpr, name, label=None):
        ...
        plt.grid(True)
        plt.savefig(os.path.join(directory, 'ROC_curve_forest_grid_ada_'+name+datestring+'.png'))
        plt.close()
    ...
    prediction_probas(self.forest_clf_grid_ada_new_database, self.X_database_train, self.y_train, self.X_database_test, self.y_test, self.database, 'database')
    prediction_probas(self.forest_clf_grid_ada_new_man_add, self.X_man_add_train, self.y_train, self.X_man_add_test, self.y_test, self.man_add, 'man_add')
    prediction_probas(self.forest_clf_grid_ada_new_transform, self.X_transform_train, self.y_train, self.X_transform_test, self.y_test, self.transform, 'transform')
    prediction_probas(self.forest_clf_grid_ada_new_prot_screen_trans, self.X_prot_screen_trans_train, self.y_train, self.X_prot_screen_trans_test, self.y_test, self.prot_screen_trans, 'prot_screen_trans')
    ...
    scoring_all(self.forest_clf_grid_ada_new_database, self.X_database_train, self.y_train, self.X_database_test, self.y_test, self.database)
    scoring_all(self.forest_clf_grid_ada_new_man_add, self.X_man_add_train, self.y_train, self.X_man_add_test, self.y_test, self.man_add)
    scoring_all(self.forest_clf_grid_ada_new_transform, self.X_transform_train, self.y_train, self.X_transform_test, self.y_test, self.transform)
    scoring_all(self.forest_clf_grid_ada_new_prot_screen_trans, self.X_prot_screen_trans_train, self.y_train, self.X_prot_screen_trans_test, self.y_test, self.prot_screen_trans)
...
def run():
  ...
  random_forest_ada_grid_search = RandomForestAdaGridSearch(metrix, database, man_add, transform, prot_screen_trans)
