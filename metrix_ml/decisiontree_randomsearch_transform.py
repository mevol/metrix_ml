...
class DecisionTreeRandomSearchTransform(object):
  ...
  def prepare_metrix_data(self):
    ...
    ******
    ...
    with open(os.path.join(self.transform_only, 'decisiontree_randomsearch.txt'), 'a') as text_file:
...
#    X_transform_train, X_transform_test, y_train, y_test = train_test_split(self.X_transform, y, test_size=0.2, random_state=42, stratify=y)
  ...
  def rand_search(self):
    ...
    param_rand = {"criterion": ["gini", "entropy"],#metric to judge reduction of impurity
                  'max_features': randint(2, 39),#max number of features when splitting
  ...
  def tree_best_params(self):
    ...
    plot_features(self.tree_clf_rand_new_transform, self.X_transform_train.columns, self.transform_only)
    ...
    write_pickle(self.tree_clf_rand_new_transform, self.transform_only, 'transform')
    ...
    visualise_tree(self.tree_clf_rand_new_transform, self.transform_only, self.X_transform_train.columns, 'transform')
    ...
    basic_stats(self.tree_clf_rand_new_transform, self.X_transform_train, self.transform_only)
...
#      precision_recall(self.y_train, y_train_CV_pred_proba, 'train_CV_', directory)
    ...
    prediction_probas(self.tree_clf_rand_new_transform, self.X_transform_train, self.y_train, self.X_transform_test, self.y_test, self.y_pred_proba_transform, self.y_train_CV_pred_proba_transform, self.transform_only, 'transform')    
    ...
    scoring_all(self.tree_clf_rand_new_transform, self.X_transform_train, self.y_train, self.X_transform_test, self.y_test, self.transform_only)
...
def run():
  ...
  decision_tree_random_search_transform = DecisionTreeRandomSearchTransform(metrix, transform_only, bbbb)
